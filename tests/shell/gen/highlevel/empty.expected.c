...
{
	return ksNew (1,
	keyNew ("/", ELEKTRA_KEY_META, "mountpoint", "tests_gen_elektra_empty.ini", ELEKTRA_KEY_END),
	ELEKTRA_KS_END);
;
}
				 ...
				 ElektraError ** error)
{
	...
	keyNew ("system:/elektra/contract/highlevel/check/spec/token", ELEKTRA_KEY_VALUE, "15923d95dcd350f34919d052c8a262e284e56725db3557eef24b69368ca9d5e0", ELEKTRA_KEY_END),
	...
	ELEKTRA_KS_END);
;
	ElektraKey * parentKey = keyNew ("/tests/script/gen/highlevel/empty", ELEKTRA_KEY_END);
...
void exitForSpecload (int argc, const char * const * argv)
{
	...
	ElektraKey * parentKey = keyNew ("spec:/tests/script/gen/highlevel/empty", ELEKTRA_KEY_META, "system:/elektra/quickdump/noparent", "", ELEKTRA_KEY_END);
