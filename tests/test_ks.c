...
void test_ksDuplicate()
{
	...
	succeed_if (ksAppendKey (ks, keyNew("system/duplicate", KEY_END)) == 1, "could not append key");
	succeed_if (ksAppendKey (ks, keyNew("system/duplicate", KEY_END)) == -1, "could append duplicate key");

	ksRewind (ks);
	for (i=ksNext(ks); i!=0; i=ksNext(ks))
	{
		printf ("%s", keyName(i));
	}
