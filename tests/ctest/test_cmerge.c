...
static void simple_test (char * our_value, char * their_value, char * base_value, int strategy, char * expected_result)
{
	...
	{
		ElektraKey * resultKey = ksLookupByName (result, "user:/result/key", 0);
		if (resultKey == NULL)
		{
			char msg[200];
			snprintf (msg, 200,
...
static void test_order (char * our_order, char * their_order, char * base_order, int strategy, char * expected_result)
{
	...
	{
		const ElektraKey * resultKey = ksLookupByName (result, "user:/result/key", 0);
		if (resultKey == NULL)
		{
			yield_error ("Lookup must succeed");
		}
		const ElektraKey * metaKey = keyGetMeta (resultKey, "order");
		if (metaKey == NULL)
		{
