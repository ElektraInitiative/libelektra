			...
			if (PLUGINPROCESS_FOUND)
				if (HASKELL_FOUND)
					...
					if (HAVE_HASKELL_BINDING)
						...
						if (BUILD_SHARED OR BUILD_FULL) # shared variants of ghc libraries have the ghc version as a
										# suffix
								...
								if (GHC_GMP_LIB)
									if (GHC_PRIM_LIB)
										...
										configure_file (
											...
											@ONLY) # copy the readme so the macro in haskell.c
											       # finds it
										file (COPY
										      "${CMAKE_CURRENT_SOURCE_DIR}/README.md"
										      DESTINATION
										      "${CMAKE_CURRENT_BINARY_DIR}") # same for the setup
														     # logic,  depending on
														     # wheter a custom  one
														     # exists  use the
										...
										# as we require the sandbox to exist we can do this during
										# the configuration phase it doesn't compile or install
										# anything
										execute_process (COMMAND ${CABAL_EXECUTABLE} sandbox init
													 --sandbox
													 "${HASKELL_SHARED_SANDBOX}" -v0
												 WORKING_DIRECTORY
										...
										# reconfiguration due to Cabal library version issues on
										# stretch
