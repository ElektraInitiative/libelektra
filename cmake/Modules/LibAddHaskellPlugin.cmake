...
macro (add_haskell_plugin target)
					...
					if (HASKELL_FOUND)
						...
						if (HAVE_HASKELL_BINDING)
							...
							set (GHC_INCLUDE_DIRS
							     ${CMAKE_CURRENT_BINARY_DIR}/dist/build/Elektra
							     ${GHC_LIB_DIR}/include)

							set (CABAL_OPTS "--prefix=${CMAKE_INSTALL_PREFIX}")
							if (BUILD_SHARED OR BUILD_FULL)

								# shared variants of ghc libraries have the ghc
								# version as a suffix
							...
							find_library (GHC_BASE_LIB
								      "HS${GHC_BASE_NAME}${GHC_DYNAMIC_SUFFIX}"
								      PATHS ${GHC_BASE_PATH})
							...
							find_library (GHC_GMP_LIB
								      "HS${GHC_GMP_NAME}${GHC_DYNAMIC_SUFFIX}"
								      PATHS ${GHC_GMP_PATH})
							...
							find_library (GHC_PRIM_LIB
								      "HS${GHC_PRIM_NAME}${GHC_DYNAMIC_SUFFIX}"
								      PATHS ${GHC_PRIM_PATH})
								...
								if (GHC_BASE_LIB)
									if (GHC_GMP_LIB)
										if (GHC_PRIM_LIB)
											...
											# configure the haskell plugin base file for the
											# current plugin
											configure_file (
											...
											# same for the setup logic, depending on wheter a
											# custom one exists use the default suitable for
											# almost everything
											...
											# as we require the sandbox to exist we can do this
											# during the configuration phase it doesn't compile
											# or install anything
											execute_process (COMMAND ${CABAL_EXECUTABLE} sandbox
														 init
														 --sandbox
											...
											foreach (SANDBOX_ADD_SOURCE
												 ${ARG_SANDBOX_ADD_SOURCES})

												# our custom libs are all to be processed by
												# cmake before we can add them, so enforce
												# that, the build is more stable this way
												if (
													NOT
													IS_DIRECTORY
											...
											get_property (HASKELL_LAST_PLUGIN
												      GLOBAL
												      PROPERTY HASKELL_LAST_PLUGIN)
											...
											# reconfiguration due to Cabal library version
											# issues on stretch
											add_custom_command (
												OUTPUT ${PLUGIN_HASKELL_NAME}
	...
	if (ADDTESTING_PHASE AND TARGET elektra-${target})
		...
		# this is required so that it finds the type checker plugin
