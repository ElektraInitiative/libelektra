 ...
 * @ingroup backend
 */
int kdbGetFileName (Key * forKey, char * filename, char * keyname)
{
	char * name;
	char * end;
	size_t namesize;
...
int kdbGetKey_ini(Key *key) {
	...
	pos = kdbGetFileName(key, keyFileName, keyName);

	if (! pos) {
		printf ("Could not receive filename");
		return -1;
	}
	printf ("Get Key %s in File: %s\n", keyName, keyFileName);
	...
	while ((r=read (fd, buffer, BUFFER_SIZE)) != -1)
	{
		...
		if (	strcmp (buffer_key, keyName) == 0 &&
			(state == STATE_END)) break;
	}
	
	if (strcmp (buffer_key, keyName) != 0) {	// falscher Key
		printf ("Key not found\n");
		return -1;
	}
	
	printf ("key: %s, value: %s, comment: %s\n", 
		buffer_key, buffer_value, buffer_comment);
