			  ...
			  ElektraKeyset * goptsConfig)
{
	...
	elektraKeysetAppendKey (contract, elektraKeyNew ("system:/elektra/contract/mountglobal/gopts", ELEKTRA_KEY_END));
	if (goptsConfig != NULL)
	{
		ElektraKey * configRoot = elektraKeyNew ("user:/", ELEKTRA_KEY_END);
		ElektraKey * contractRoot = elektraKeyNew ("system:/elektra/contract/mountglobal/gopts", ELEKTRA_KEY_END);

		elektraCursor end;
		for (elektraCursor it = elektraKeysetFindHierarchy (goptsConfig, configRoot, &end); it < end; it++)
		{
			ElektraKey * renamed = elektraKeyDup (elektraKeysetAtCursor (goptsConfig, it), ELEKTRA_KEY_CP_ALL);
			elektraKeyReplacePrefix (renamed, configRoot, contractRoot);
			elektraKeysetAppendKey (contract, renamed);
		}

		elektraKeyDel (configRoot);
		elektraKeyDel (contractRoot);
	}

	elektraKeysetAppendKey (contract, elektraKeyNew ("system:/elektra/contract/globalkeyset/gopts/parent", ELEKTRA_KEY_VALUE, elektraKeyName (parentKey), ELEKTRA_KEY_END));
	if (argc != 0)
	{
		elektraKeysetAppendKey (contract, elektraKeyNew ("system:/elektra/contract/globalkeyset/gopts/argc", ELEKTRA_KEY_BINARY, ELEKTRA_KEY_SIZE, sizeof (int),
					       ELEKTRA_KEY_VALUE, &argc, ELEKTRA_KEY_END));
		elektraKeysetAppendKey (contract, elektraKeyNew ("system:/elektra/contract/globalkeyset/gopts/argv", ELEKTRA_KEY_BINARY, ELEKTRA_KEY_SIZE,
					       sizeof (const char * const *), ELEKTRA_KEY_VALUE, &argv, ELEKTRA_KEY_END));
	}

	if (envp != NULL)
	{
		elektraKeysetAppendKey (contract, elektraKeyNew ("system:/elektra/contract/globalkeyset/gopts/envp", ELEKTRA_KEY_BINARY, ELEKTRA_KEY_SIZE,
				     ...
				     const ElektraKey * parentKey, ElektraKeyset * goptsConfig)
{
	...
	if (args != NULL)
	{
		elektraKeysetAppendKey (contract, elektraKeyNew ("system:/elektra/contract/globalkeyset/gopts/args", ELEKTRA_KEY_BINARY, ELEKTRA_KEY_SIZE, argsSize,
					       ELEKTRA_KEY_VALUE, args, ELEKTRA_KEY_END));
	}

	if (env != NULL)
	{
		elektraKeysetAppendKey (contract, elektraKeyNew ("system:/elektra/contract/globalkeyset/gopts/env", ELEKTRA_KEY_BINARY, ELEKTRA_KEY_SIZE, envSize, ELEKTRA_KEY_VALUE,
					       env, ELEKTRA_KEY_END));
	}
	return 0;
}
