 ...
 */
ssize_t keyGetUnescapedName (const ElektraKey * key, char * returnedName, size_t maxSize)
{
	if (!key) return -1;
	if (!returnedName) return -1;
 ...
 */
static size_t keyAddBaseNameInternal (ElektraKey * key, const char * baseName)
{
