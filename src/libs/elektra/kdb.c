...
KDB * kdbOpen (Key * errorKey)
{
	if (!errorKey)
	{
		ELEKTRA_LOG (ELEKTRA_LOG_MODULE_KDB, "no error key passed");
		return 0;
	}

	ELEKTRA_LOG (ELEKTRA_LOG_MODULE_KDB, "called with %s", keyName (errorKey));
	...
	case 0:
		...
		break;
	case 2:
		ELEKTRA_LOG (ELEKTRA_LOG_MODULE_KDB, "entered fallback code for bootstrapping");
		inFallback = 1;
		break;
	}
	...
	if (elektraMountGlobals (handle, ksDup (keys), handle->modules, errorKey) == -1)
	{
		...
		ELEKTRA_LOG_WARNING (ELEKTRA_LOG_MODULE_KDB, "Mounting global plugins failed");
	}
...
#ifdef HAVE_LOGGER
	if (inFallback)
		ELEKTRA_LOG_WARNING (ELEKTRA_LOG_MODULE_KDB, "fallback for bootstrapping: you might want to run `kdb upgrade-bootstrap`");
	...
	{
		ELEKTRA_LOG_DEBUG (ELEKTRA_LOG_MODULE_KDB, "config for createTrie name: %s value: %s", keyName (key), keyString (key));
	}
#endif
...
int kdbGet (KDB * handle, KeySet * ks, Key * parentKey)
{
	...
	ELEKTRA_LOG (ELEKTRA_LOG_MODULE_KDB, "now in new kdbGet (%s)", keyName (parentKey));
