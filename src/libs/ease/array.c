...
/**
 * @brief Read all the array values as strings below the given array parent
 *
 * For example, if `user/config/#` is an array,
 * `user/config` is the array parent.
 *
 * An array is created using elektraCalloc() and elements are copied into it using elektraStrDup().
 * This means the caller has to call elektraFree() on the array passed as @p strings as well as all its elements.
 *
 * @param arrayParent the parent of the array to be returned
 * @param keys the keyset containing the array keys
 * @param strings a pointer to where the resulting array should be stored
 *
 * @return the length of the resulting array stored in @p strings
 * @retval -1 on `NULL` pointers
 */
ssize_t elektraArrayGetStrings (const Key * arrayParent, KeySet * keys, char *** strings)
{
	KeySet * elements = elektraArrayGet (arrayParent, keys);

	ssize_t size = ksGetSize (elements);
	if (size <= 0)
	{
		*strings = NULL;
		return size;
	...
	*strings = elektraCalloc (size * sizeof (char *));
