...
TEST (SpecReader, withDatabase)
{
	...
	SpecReader sr (mpi);
	sr.readSpecification (KeySet (5, *Key ("spec:/", ELEKTRA_KEY_END), *Key ("spec:/mp", ELEKTRA_KEY_META, "mountpoint", "file.ini", ELEKTRA_KEY_END),
				      *Key ("spec:/mp/below", ELEKTRA_KEY_META, "config/needs/something", "here", ELEKTRA_KEY_END), ELEKTRA_KS_END));
	SpecBackendBuilder bi = sr.getBackends ()[Key ("spec:/mp", ELEKTRA_KEY_END)];
	EXPECT_EQ (bi.nodes, 2);
	EXPECT_EQ (bi.getMountpoint (), "/mp");
	EXPECT_EQ (bi.getConfigFile (), "file.ini");
	EXPECT_EQ (bi.getBackendConfig (), KeySet (5, *Key ("user:/something", ELEKTRA_KEY_VALUE, "here", ELEKTRA_KEY_END), ELEKTRA_KS_END));
...
TEST (SpecReader, withDatabaseRecursive)
{
	...
	SpecReader sr (mpi);
	sr.readSpecification (KeySet (5, *Key ("spec:/", ELEKTRA_KEY_END), *Key ("spec:/mp", ELEKTRA_KEY_META, "mountpoint", "file.ini", ELEKTRA_KEY_END),
				      *Key ("spec:/mp/below", ELEKTRA_KEY_META, "config/needs/something", "else", ELEKTRA_KEY_END),
				      *Key ("spec:/mp/below/recursive", ELEKTRA_KEY_META, "mountpoint", "otherfile.ini", ELEKTRA_KEY_END), ELEKTRA_KS_END));
	SpecBackendBuilder bi = sr.getBackends ()[Key ("spec:/mp", ELEKTRA_KEY_END)];
	EXPECT_EQ (bi.nodes, 2);
}
...
TEST (SpecReader, withNeeds)
{
	...
	sr.readSpecification (KeySet (
		5, *Key ("spec:/", ELEKTRA_KEY_END), *Key ("spec:/mp", ELEKTRA_KEY_META, "mountpoint", "file.ini", ELEKTRA_KEY_END),
		*Key ("spec:/mp/below", ELEKTRA_KEY_META, "config/needs/something", "here", ELEKTRA_KEY_META, "infos/needs", "resolver storage", ELEKTRA_KEY_END),
		ELEKTRA_KS_END));
	SpecBackendBuilder bi = sr.getBackends ()[Key ("spec:/mp", ELEKTRA_KEY_END)];
	EXPECT_EQ (bi.nodes, 2);
	EXPECT_EQ (bi.getMountpoint (), "/mp");
...
TEST (SpecReader, withNeedsResolved)
{
	...
	sr.readSpecification(KeySet(5,
				*Key ("spec:/", ELEKTRA_KEY_END),
				*Key ("spec:/mp", ELEKTRA_KEY_META, "mountpoint", "file.ini",
					ELEKTRA_KEY_META, "config/needs/something", "here",
					ELEKTRA_KEY_META, "infos/needs", "resolver storage",
					ELEKTRA_KEY_END),
				*Key ("spec:/mp/below",
					ELEKTRA_KEY_META, "config/needs/else", "too",
					ELEKTRA_KEY_META, "infos/needs", "a b",
					ELEKTRA_KEY_END),
				ELEKTRA_KS_END));
	// clang-format on
	SpecBackendBuilder bi = sr.getBackends ()[Key ("spec:/mp", ELEKTRA_KEY_END)];
	...
	EXPECT_EQ (bi.getMountpoint (), "/mp");
	EXPECT_EQ (bi.getConfigFile (), "file.ini");
	EXPECT_EQ (bi.getBackendConfig (), KeySet (5, *Key ("user:/something", ELEKTRA_KEY_VALUE, "here", ELEKTRA_KEY_END),
						   *Key ("user:/else", ELEKTRA_KEY_VALUE, "too", ELEKTRA_KEY_END), ELEKTRA_KS_END));
	bi.resolveNeeds ();
	ASSERT_EQ (std::distance (bi.begin (), bi.end ()), 2) << "there should be a resolver and storage added";
...
TEST (SpecReader, withNeedsResolvedPreferencesPlugins)
{
	...
	sr.readSpecification(KeySet(5,
				*Key ("spec:/", ELEKTRA_KEY_END),
				*Key ("spec:/mp", ELEKTRA_KEY_META, "mountpoint", "file.ini",
					ELEKTRA_KEY_META, "infos/plugins", "b",
					ELEKTRA_KEY_META, "infos/needs", "resolver storage",
					ELEKTRA_KEY_END),
				*Key ("spec:/mp/below",
					ELEKTRA_KEY_END),
...
TEST (SpecReader, withNeedsResolvedNumerical)
{
	...
	sr.readSpecification (
		KeySet (5, *Key ("spec:/", ELEKTRA_KEY_END),
			*Key ("spec:/mp", ELEKTRA_KEY_META, "mountpoint", "file.ini", ELEKTRA_KEY_META, "infos/needs", "resolver storage", ELEKTRA_KEY_END),
			*Key ("spec:/mp/below", ELEKTRA_KEY_END), ELEKTRA_KS_END));
	SpecBackendBuilder bi = sr.getBackends ()[Key ("spec:/mp", ELEKTRA_KEY_END)];
	EXPECT_EQ (bi.nodes, 2);
	EXPECT_EQ (bi.getMountpoint (), "/mp");
	EXPECT_EQ (bi.getConfigFile (), "file.ini");
...
TEST (SpecReader, withNeedsResolvedPreferencesIgnored)
{
	...
	sr.readSpecification(KeySet(5,
				*Key ("spec:/", ELEKTRA_KEY_END),
				*Key ("spec:/mp", ELEKTRA_KEY_META, "mountpoint", "file.ini",
					ELEKTRA_KEY_META, "infos/needs", "a", // warning: order matters here..
					ELEKTRA_KEY_END),
				*Key ("spec:/mp/below",
					ELEKTRA_KEY_META, "infos/needs", "resolver storage",
					ELEKTRA_KEY_END),
...
TEST (SpecReader, withMetadata)
{
	...
	sr.readSpecification(KeySet(5,
				*Key ("spec:/", ELEKTRA_KEY_END),
				*Key ("spec:/mp", ELEKTRA_KEY_META, "mountpoint", "file.ini",
					ELEKTRA_KEY_META, "rename/toupper", "2",
					ELEKTRA_KEY_END),
				*Key ("spec:/mp/below",
					ELEKTRA_KEY_META, "default", "5",
					ELEKTRA_KEY_META, "check/math", "below >= 3",
					ELEKTRA_KEY_END),
				ELEKTRA_KS_END));
	// clang-format on
	SpecBackendBuilder bi = sr.getBackends ()[Key ("spec:/mp", ELEKTRA_KEY_END)];
...
TEST (SpecReader, withMetadataPreference)
{
	...
	SpecReader sr (mpi);
	sr.readSpecification (KeySet (5, *Key ("spec:/", ELEKTRA_KEY_END), *Key ("spec:/mp", ELEKTRA_KEY_META, "mountpoint", "file.ini", ELEKTRA_KEY_END),
				      *Key ("spec:/mp/below", ELEKTRA_KEY_META, "check/math", "below >= 3", ELEKTRA_KEY_END), ELEKTRA_KS_END));
	SpecBackendBuilder bi = sr.getBackends ()[Key ("spec:/mp", ELEKTRA_KEY_END)];
	EXPECT_EQ (bi.nodes, 2);
	EXPECT_EQ (bi.getMountpoint (), "/mp");
	EXPECT_EQ (bi.getConfigFile (), "file.ini");
...
TEST (SpecReader, pluginConfiguration)
{
	...
	sr.readSpecification(KeySet(5,
				...
				*Key ("spec:/mp/transform",
					ELEKTRA_KEY_META, "infos/plugins", "python#transform script=transform.py",
					ELEKTRA_KEY_META, "transform/python", "below = other+5",
					ELEKTRA_KEY_END),
				*Key ("spec:/mp/rename",
					ELEKTRA_KEY_META, "infos/plugins", "lua#rename norename=,script=rename.lua",
					ELEKTRA_KEY_META, "rename/toupper", "1",
					ELEKTRA_KEY_END),
				ELEKTRA_KS_END));
	// clang-format on
	SpecBackendBuilder bi = sr.getBackends ()[Key ("spec:/mp", ELEKTRA_KEY_END)];
	...
	EXPECT_EQ (bi.getConfigFile (), "file.ini");
	ASSERT_EQ (std::distance (bi.begin (), bi.end ()), 2) << "there should be plugins added";
	EXPECT_EQ (bi.begin ()[0], PluginSpec ("lua#rename", KeySet (2, *Key ("user:/script", ELEKTRA_KEY_VALUE, "rename.lua", ELEKTRA_KEY_END),
								     *Key ("user:/norename", ELEKTRA_KEY_END), ELEKTRA_KS_END)));
	EXPECT_EQ (bi.begin ()[1],
		   PluginSpec ("python#transform", KeySet (1, *Key ("user:/script", ELEKTRA_KEY_VALUE, "transform.py", ELEKTRA_KEY_END), ELEKTRA_KS_END)));
	bi.resolveNeeds ();
	ASSERT_EQ (std::distance (bi.begin (), bi.end ()), 2) << "there should be plugins added";
	EXPECT_EQ (bi.begin ()[0], PluginSpec ("lua#rename", KeySet (2, *Key ("user:/script", ELEKTRA_KEY_VALUE, "rename.lua", ELEKTRA_KEY_END),
								     *Key ("user:/norename", ELEKTRA_KEY_END), ELEKTRA_KS_END)));
