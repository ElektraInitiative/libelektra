...
{
	ret.append (*Key (baseKey.getName () + "/errorplugins", KEY_META, "comment", "List of plugins to use", KEY_END));
	...
	for (int i = 0; i < NR_OF_PLUGINS; ++i)
	{
		...
		ret.append (*Key (name, KEY_META, "comment", "A plugin", KEY_END));
...
{
	ret.append (*Key (baseKey.getName () + "/getplugins", KEY_META, "comment", "List of plugins to use", KEY_END));
...
{
	ret.append (*Key (baseKey.getName () + "/setplugins", KEY_META, "comment", "List of plugins to use", KEY_END));
