...
long long iterations = 10LL; // elitebenchmark sync/reload benchmark
...
__attribute__ ((noinline)) void benchmark_layer_syncN (kdb::Environment &, long long N)
{
	kdb::Coordinator c;
	kdb::ThreadContext tc (c);
	kdb::KeySet ks;
...
__attribute__ ((noinline)) void benchmark_kdb_reloadN (kdb::Environment &, long long N)
{
	kdb::Coordinator c;
	kdb::ThreadContext tc (c);
	kdb::KeySet ks;
	...
	kdb::KDB kdbx[iterations]; // really expensive operation, but need to be clean for every benchmark
...
__attribute__ ((noinline)) void benchmark_layer_reload (kdb::Environment & s)
{
	s.nested = value;
	kdb::ThreadInteger::type x = 0;

	static Timer t ("layer reload");
	t.start ();
	//{benchmark/layerswitch}
	for (long long i = 0; i < iterations / 2; ++i)
	{
...
int main (int argc, char ** argv)
{
	...
	for (int i = 0; i < benchmarkIterations; ++i)
	{
		...
		*/
		for (int j = 0; j < 10; ++j)
		{
			benchmark_layer_syncN (s, j);
			benchmark_kdb_reloadN (s, j);
		}
	...
	for (int i = 0; i<10; ++i)
	{
		std::cerr << i << "," << tSync[i]->getMedian() << "," << tReload[i]->getMedian() << std::endl;
	}
