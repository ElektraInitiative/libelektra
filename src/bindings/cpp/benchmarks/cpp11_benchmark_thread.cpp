...
public:
	...
	ThreadInteger(ks,context_, Key("/test/%layer1%/%thread%/%layer2%/nested",
					KEY_CASCADING_NAME,
					 KEY_META, "default", s_value,
					 KEY_END))
	{}
...
public:
	Environment(KeySet & ks, ThreadContext &context_) :
		...
		bm(ks,context_, Key("/%layer1%/%layer2%/%layer3%/%layer4%/%layer5%/%layer6%/%layer7%/%layer8%/%layer9%/",
					KEY_CASCADING_NAME,
					KEY_META, "default", s_value,
					KEY_END))
	{}
...
kdb::ThreadInteger::type add_native(uint32_t const & i1,
				uint32_t const & i2)
{ return i1+i2; }

kdb::ThreadInteger::type add_contextual(kdb::ThreadInteger const & i1,
					kdb::ThreadInteger const & i2)
{ return i1+i2; }
...
__attribute__((noinline)) void benchmark_kslookup()
{
	...
	kdb::KeySet ks( 100,
					*kdb::Key("user/hello/some", KEY_END),
					*kdb::Key("user/hello/a/key", KEY_END),
					*kdb::Key("user/hello/b/key", KEY_END),
					*kdb::Key("user/hello/c/key", KEY_END),
					*kdb::Key("user/hello/d/key", KEY_END),
					*kdb::Key("user/other", KEY_END),
					KS_END);
	t.start();
	for (long long i=0; i<iterations; ++i)
	{
