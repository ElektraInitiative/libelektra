...
void ckdb::read()
{
	...
	if (kdbGetChildKeys(handle, root.c_str(), user_keys, KDB_O_RECURSIVE))
	{
	...
	ksRewind(user_keys);
	for (current=ksCurrent(user_keys);;current = ksNext (user_keys)) 
	{
		if (current = 0) break;
...
Key * ckdb::getKey (std::string key)
{
	...
	if (add_keys.find(key) != add_keys.end())	// does element exists?
	{
		...
		k = keyNew("", KEY_SWITCH_END);  	// generate new key
		add_keys[key] = k;
		keyInit (k);
