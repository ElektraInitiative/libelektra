...
TEST_F(AutoMergeStrategyTest, EqualsModifyMerges)
{
	...
	EXPECT_EQ(mergeKeys.at (1), merged.at (1));
...
TEST_F(AutoMergeStrategyTest, AddEqualsKeyMerge)
{
	...
	mergeKeys.append (Key ("user/parentm/config/key5", KEY_VALUE, "value5", KEY_END));
	Key conflictKey = mergeKeys.lookup ("user/parentm/config/key5");
