...
/**
 * @defgroup moremeta Proposals for Meta Data
 *
 * @brief Meta data compatibility methods
 *
 * In versions before Elektra 0.8 only specific meta data was available.
 * Now any meta data can be added. These API methods are implementations
 * of the 0.7 API using 0.8 meta data.
 *
 * These methods might be moved to another library and/or generated by the descriptions in
 * doc/METADATA.ini.
 *
