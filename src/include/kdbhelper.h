...
{
	ELEKTRA_KDB_O_SPEC = 1 << 15,	     ///< Use the passed key as specification, instead of looking up the specification first
	ELEKTRA_KDB_O_CREATE = 1 << 16,	     ///< Create the key if it was not found
	ELEKTRA_KDB_O_NOCASCADING = 1 << 17, ///< Disable cascading search for keys starting with /
	ELEKTRA_KDB_O_NOSPEC = 1 << 18,	     ///< Do not use specification for cascading keys (internal)
	ELEKTRA_KDB_O_NODEFAULT = 1 << 19,   ///< Do not honor the default spec (internal)
	ELEKTRA_KDB_O_CALLBACK = 1 << 20,    ///< For spec:/ lookups that traverse deeper into hierarchy (callback in ksLookup())
	ELEKTRA_KDB_O_OPMPHM = 1 << 21,	  ///< Overrule ksLookup search predictor to use OPMPHM, make sure to set ENABLE_OPTIMIZATIONS=ON at cmake
	ELEKTRA_KDB_O_BINSEARCH = 1 << 22 ///< Overrule ksLookup search predictor to use Binary search for lookup
};
