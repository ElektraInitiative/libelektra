...
module.exports = function ($scope, Logger, formats, ConversionService, ReportService, Notification) {
	...
	this.createGithubIssue = function () {
		var title = "Convert with plugin " + $scope.parameters.input.format.plugin.name + " to " +
			    $scope.parameters.output.format.plugin.name;
		var message = "I tried to convert a snippet on the website, but got the error: " + $scope.lastError + "\n\n" +
			      "## Used plugins\n" +
			      "```\n" +
			      "Input:\n" +
