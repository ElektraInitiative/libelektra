...
module.exports = function ($scope, $rootScope, Logger, $state, Notification, UserService, $stateParams, user) {
	...
	$scope.ranks = [
		{ id: 10, name: "APP.USERS.DETAILS.LABEL.RANK.10" }, { id: 50, name: "APP.USERS.DETAILS.LABEL.RANK.50" },
	...
	this.changePassword = function () {
		...
		vm.executeChange({ password: $scope.changes.password.input1 }, $scope.passwordForm, function () {
	...
	this.changeEmail = function () {
		...
		vm.executeChange({ email: $scope.changes.email }, $scope.emailForm, function () {
	...
	this.changeRank = function () {
		...
		vm.executeChange({ rank: $scope.changes.rank.id }, $scope.rankForm, function () {
			$scope.user.rank = $scope.changes.rank.id;
		});
	};

	this.executeChange = function (data, form, callback) {
		UserService.update($scope.user.username, data, $stateParams.useAuthUser)
			.then(
				function (response) {
					callback ();
