...
module.exports = function ($rootScope, $scope, Logger, $state, $anchorScroll, webStructure, config, $translate) {
	...
	// build date
	$scope.builddate = { date: new Date (document.querySelector('meta[name="build-date"]').getAttribute("content")) };
	$scope.builddate.timezoneOffset = $scope.builddate.date.getTimezoneOffset() / -60;
	$scope.builddate.pretty = $scope.builddate.date.toLocaleString() + " (" + ($scope.builddate.timezoneOffset >= 0 ? "+" : "") +
				  $scope.builddate.timezoneOffset + "h UTC)";
