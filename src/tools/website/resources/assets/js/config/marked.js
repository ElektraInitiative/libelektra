...
module.exports = function (markedProvider, config, webStructure) {
	...
	markedProvider.setRenderer({
		...
		link: function (href, title, text) {
			...
			if (href.indexOf("://") > -1)
			{
				return ('<a href="' + href + '"' + (title ? ' title="' + title + '"' : "") + ' target="_blank">' + text +
					' <i class="fa fa-external-link"></i></a>');
			}
			// anchor link
			if (href.indexOf("#") > -1)
			{
				return ('<a href="' + href + '"' + (title ? ' title="' + title + '"' : "") + " >" + text +
					' <i class="fa fa-external-link"></i></a>');
			}
			// internal link
		...
		image: function (href, title, text) {
			...
			return ('<img src="' + config.website.content_root + href + '"' + (text ? ' alt="' + text + '"' : "") +
				(title ? ' title="' + title + '"' : "") + "/>");
		}
	});
