...
module.exports = function (Logger, $http, $q, config) {
	...
	this.cache = {
		formats: [],
		typeaheads: { cached: false, data: { organizations: [], applications: [], scopes: [], tags: [] } },
		search: { cached: false, entries: {}, params: { filter: "", filterby: "", sort: "", sortby: "", rows: 0, offset: 0 } }
	};
	...
	this.search = function (params, force) {
		...
		if (!service.cache.search.cached || !angular.equals(service.cache.search.params, params) || force)
		{
			...
			$http.get(config.backend.root + "database", { params: params })
				.then(
					function (response) {
					...
					function (response) {
						if (response.data.elements > 0 && typeof response.data.entries !== "undefined")
						{
							response.data.entries.forEach(function (entry) {
								if (service.cache.typeaheads.data.organizations.indexOf(
									    entry.key.organization) === -1)
								{
									service.cache.typeaheads.data.organizations.push(
										entry.key.organization);
								}
								if (service.cache.typeaheads.data.applications.indexOf(
									    entry.key.application) === -1)
								{
