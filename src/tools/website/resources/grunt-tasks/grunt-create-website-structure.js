...
module.exports = function (grunt) {
	grunt.registerMultiTask("create-website-structure", "Builds a comprehensive website structure file.", function () {
		...
		this.handleSubmenuEntry = function (entry) {
			var result = { name: entry.name, type: "submenu", ref: entry.ref, children: [] };
		...
		this.handleLinkEntry = function (entry) {
			var result = { name: entry.name, type: "link", ref: entry.ref, options: { path: entry.options.path } };
			return result;
		};

		this.handleParsereadmeEntry = function (entry) {
			var result = { name: entry.name, type: "listfiles", ref: entry.ref, children: [] };
		...
		this.handleListdirsEntry = function (entry) {
			...
			directories.forEach(function (dir) {
				...
				if (file !== null)
				{
					var fileEntry = { name: self.makePrettyName(dir.name), type: "file", options: { path: file } };
					fileEntry.slug = self.createSlugFromName(fileEntry.name);
					result.children.push(fileEntry);
				}
			});
		...
		this.handleStaticfileEntry = function (entry) {
			var result = { name: entry.name, type: "file", options: { path: path.join(entry.options.path) } };
			result.slug = self.createSlugFromName(result.name);
			return result;
		};

		this.handleStaticrefEntry = function (entry) {
			var result = { name: entry.name, type: "ref", options: { path: entry.options.path } };
			return result;
		};
		...
		this.listDirectory = function (relPath) {
			...
			entries.forEach(function (entry) {
				result.push({ name: entry, stats: fs.statSync(path.join(root_dir, relPath, entry)) });
			});
			return result;
		};
