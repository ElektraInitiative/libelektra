...
module.exports = function (grunt) {
	...
	grunt.initConfig({
		...
		global: { repository: { root: "@REST_FRONTEND_SOURCE_INSTALL_REPOSITORY@" } },
		...
		"create-website-news": {
			...
			build: {
				...
				regex: {
					filename: { pattern: "^([0-9]{4}\\-[0-9]{2}\\-[0-9]{2})_(.*)$", flags: "i" },
					title: { pattern: "^# ([^#\n]*)$", flags: "im" },
		...
		"create-website-news-rss": {
			...
			build: {
				repo_root: "<%= global.repository.root %>",
				input: { news: "<%= grunt.config('create-website-news.build.output') %>" },
		...
		jshint: {
			options: { reporter: require ("jshint-stylish"), node: true, browserify: true },
		...
		concat: {
			...
			vendor: {
				src: [
					"public/assets/skin/bootstrap/bootstrap.min.css", "node_modules/highlight.js/styles/github.css",
			...
			docsearchcss:
				{ src: ["node_modules/docsearch.js/dist/cdn/docsearch.min.css"], dest: "public/vendor/docsearch.min.css" },
			docsearchjs:
				{ src: ["node_modules/docsearch.js/dist/cdn/docsearch.min.js"], dest: "public/vendor/docsearch.min.js" },
			pacejs: { src: ["node_modules/pace-progress/pace.min.js"], dest: "public/vendor/pace.min.js" },
			pacecss: {
		...
		copy: {
			...
			build: {
				files: [{
					...
					src: ["qtgui.png", "web-gui-kdb.png", "oyranos-kolor-manager.png", "oyranos-km-logo.svg"],
		...
		preprocess: {
			...
			config: { src: "resources/assets/js/config/index.js.in", dest: "resources/assets/js/config/index.js" },
			index: { src: "resources/assets/html/index.html.in", dest: "public/index.html" }
		},
		watch: {
			options: {},
			less: { files: ["resources/assets/skin/**/*"], tasks: ["less", "cssmin"] },
			preprocess: {
				files: [
					"application-config.json", "<%= grunt.config('create-website-structure.build.output') %>",
		...
		uglify: {
			options: { compress: true, mangle: false, sourceMap: true, screwIE8: false },
			build: { files: { "public/assets/js/application.js": ["<%= grunt.config('browserify.build.dest') %>"] } }
		},
		connect: {
			server: {
				options: {
					...
					port: 9000,
					protocol: "http",
					base: { path: "public", options: { index: "index.html", maxAge: 3600 } },
					keepalive: true,
					middleware: function (connect, options, middlewares) {
						...
						var staticExtensions = [
							"html", "js",  "css", "json", "svg",  "md",    "png", "jpg",
							"gif",	"otf", "eot", "ttf",  "woff", "woff2", "xml", "c",
							"h",	"cpp", "hpp", "java", "py",   "rss",   "ini"
						];
						return [
							modRewrite (["!" +
								     staticExtensions
	...
	grunt.registerTask("full", [
		"stylesheets", "website-news", "create-website-structure", "copy-website-content", "copy", "preprocess", "browserify:build",
		"uglify:build"
	]);
