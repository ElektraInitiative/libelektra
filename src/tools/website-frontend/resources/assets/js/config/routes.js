...
module.exports = function ($stateProvider, $urlRouterProvider, $locationProvider, webStructure) {
	...
	$stateProvider.state("main", { abstract: true, templateUrl: "pages/main/template.html", controller: "MainController as ctrl" })
		.state("main.error", {
			abstract: true,
			url: "/error",
			templateUrl: "pages/main/error/template.html",
			ncyBreadcrumb: { label: "APP.BREADCRUMBS.MAIN.ERROR", parent: "main.home" }
		})
		.state("main.error.404", {
			url: "/404",
			templateUrl: "pages/main/error/404.html",
			ncyBreadcrumb: { label: "APP.BREADCRUMBS.MAIN.ERROR.404", parent: "main.error" }
		})
		.state("main.auth", {
			abstract: true,
			url: "/auth",
		...
		.state("main.auth.login", {
			...
			ncyBreadcrumb: { label: "APP.BREADCRUMBS.MAIN.AUTH.LOGIN", parent: "main.auth" }
		})
		.state("main.auth.logout", {
			url: "/logout",
			controller: [
				"$rootScope", "$auth", "$state", "Notification",
		...
		.state("main.auth.register", {
			...
			ncyBreadcrumb: { label: "APP.BREADCRUMBS.MAIN.AUTH.REGISTER", parent: "main.auth" }
		})
		.state("main.home", {
			url: "/home",
		...
		.state("main.news", {
			...
			resolve: {
				...
				currentFile: [
					"$q", "$timeout", "$state", "$stateParams", "WebsiteService", "files",
		...
		.state("main.dyn", { abstract: true, template: "<div ui-view></div>", ncyBreadcrumb: { parent: "main.home" } })
		.state("main.account", {
			url: "/account",
			templateUrl: "pages/main/users/details.html",
			controller: "UserDetailsController as ctrl",
			params: { username: "", useAuthUser: true },
			resolve: {
				user: [
			...
			ncyBreadcrumb: { label: "APP.BREADCRUMBS.MAIN.ACCOUNT", parent: "main.home" }
		})
		.state("main.entries", {
			abstract: true,
			url: "/entries",
		...
		.state("main.entries.search", {
			...
			ncyBreadcrumb: { label: "APP.BREADCRUMBS.MAIN.ENTRIES.SEARCH", parent: "main.entries" }
		})
		.state("main.entries.details", {
			...
			resolve: {
				entry: [
					"$stateParams", "EntryService",
					function ($stateParams, EntryService) {
						return EntryService.get($stateParams.entry);
					}
				]
			},
			ncyBreadcrumb: { label: "APP.BREADCRUMBS.MAIN.ENTRIES.DETAILS", parent: "main.entries.search" }
		})
		.state("main.entries.edit", {
			...
			resolve: {
				...
				hasPermission: [
					"$rootScope", "entry", "$q",
					function ($rootScope, entry, $q) {
						var deferred = $q.defer();

						if ($rootScope.currentUser && ($rootScope.currentUser.rank >= 50 ||
			...
			ncyBreadcrumb: { label: "APP.BREADCRUMBS.MAIN.ENTRIES.EDIT", parent: "main.entries.details" }
		})
		.state("main.entries.create", {
			...
			data: { rank: 10 },
			ncyBreadcrumb: { label: "APP.BREADCRUMBS.MAIN.ENTRIES.CREATE", parent: "main.entries" }
		})
		.state("main.users", {
			abstract: true,
			url: "/users",
		...
		.state("main.users.search", {
			...
			data: { rank: 100 },
			ncyBreadcrumb: { label: "APP.BREADCRUMBS.MAIN.USERS.SEARCH", parent: "main.users" }
		})
		.state("main.users.details", {
			url: "/details/:username",
			templateUrl: "pages/main/users/details.html",
			controller: "UserDetailsController as ctrl",
			params: { username: "", useAuthUser: false },
			data: { rank: 100 },
			resolve: {
				user: [
					"$stateParams", "UserService",
			...
			ncyBreadcrumb: { label: "APP.BREADCRUMBS.MAIN.USERS.DETAILS", parent: "main.users" }
		});
	...
	function consumeEntry (entry)
	{
		...
		case "listfiles":
			$stateProvider.state("main.dyn." + entry.ref, {
						...
						function ($q, $timeout, $state, $stateParams, WebsiteService, files) {
							...
							$timeout (function () {
								...
								else
								{
									...
									{
										WebsiteService.loadFile(filtered[0].options.path)
											.then(function (data) {
												var file = filtered[0];
												file.content = data;
												deferred.resolve(file);
				...
				ncyBreadcrumb: { label: "APP.BREADCRUMBS.MAIN.DYN." + entry.ref.toUpperCase(), parent: "main.dyn" }
			});
			break;
		default:
			break;
