...
export default function initRoutes(app) {
  ...
  app
    ...
    .get((req, res) =>
      kdb
        .getAndLs(req.params[0], req.query)
        .then(output => successResponse(res, output))
        .catch(err => errorResponse(res, err))
    )
    .put((req, res) =>
      kdb
        .set(req.params[0], req.body)
        .then(output => successResponse(res, output))
        .catch(err => errorResponse(res, err))
    )
    .delete((req, res) =>
  ...
  app
    ...
    .post((req, res) =>
      kdb
        .setmeta(req.params[0], req.body.key, req.body.value)
        .then(() => res.status(204).send())
        .catch(err => errorResponse(res, err))
    )
    .delete((req, res) =>
      kdb
        .rmmeta(req.params[0], req.body.key)
        .then(() => res.status(204).send())
        .catch(err => errorResponse(res, err))
    );
}
