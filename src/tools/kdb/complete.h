/**
 * @file
 *
 * @brief Header for complete command
 *
 * @copyright BSD License (see LICENSE.md or https://www.libelektra.org)
 */

#ifndef ELEKTRA_KDB_COMPLETE_H
#define ELEKTRA_KDB_COMPLETE_H

#include <kdb.h>

/**
 * Adds options specification of complete command to @spec
 *
 * @param spec the base spec where the commands spec should be added
 */
void addCompleteSpec (KeySet * spec);

/**
 * Executes the complete command
 *
 * @param options cli options and arguments as specified in addCompleteSpec()
 * @param errorKey key where errors and warnings should be saved
 *
 * @retval 0 complete command ran without errors
 * @retval 1 errors occurred, keyGetMeta (errorKey, "error/reason") for info
 *
 */
int execCppComplete (int argc, char ** argv);

#endif // ELEKTRA_KDB_COMPLETE_H
