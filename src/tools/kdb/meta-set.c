...
int execMetaSet (KeySet * options, Key * errorKey)
{
	...
	{
		ELEKTRA_SET_CLI_ERRORF (errorKey, "could not load '%s': %s", keyName (parentKey), GET_ERR (maybeCascadingParent));
		ret = 5;
		goto cleanup;
	}
	...
	{
		ret = 5;
		ELEKTRA_SET_CLI_ERRORF (errorKey, "could not set meta-value '%s' for '%s': %s", metaName, keyName (parentKey),
					GET_ERR (parentKey));
	}
