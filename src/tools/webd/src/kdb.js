...
const safeExec = script => new Promise ((resolve, reject) => exec (script, { maxBuffer: Infinity }, (err, stdout, stderr) => {
						if (err)
						{
...
const escapeValues = (template, ...values) => template.reduce((acc, part, i) => {
	/*
...
// get Elektra version
const version = () => safeExec (`${KDB_COMMAND} --version`)
			      .then(output => {
				      // parse result
...
const ls = path => safeExec (escapeValues`${KDB_COMMAND} ls -0 ${path}`).then(stdout => stdout && stdout.split("\0"));

// find paths given a search query
const find = query =>
	safeExec (escapeValues`${KDB_COMMAND} find -0 ${query}`).then(stdout => stdout && stdout.split("\0")).then(res => res || []);

// get value from given `path`
const get = path => safeExec (escapeValues`${KDB_COMMAND} get ${path}`);

// set value at given `path`
const set = (path, value) => safeExec (escapeValues`${KDB_COMMAND} set -vd ${path} -- ${value}`);

// move value from given `path` to `destination`
const mv = (path, destination) => safeExec (escapeValues`${KDB_COMMAND} mv -r ${path} ${destination}`);

// copy value from given `path` to `destination`
const cp = (path, destination) => safeExec (escapeValues`${KDB_COMMAND} cp -r ${path} ${destination}`);
...
const rm = path => {
	return ls (path)
		...
		.then(restricted => Promise.all(restricted.map(({ p, r }) => {
			if (r !== "1") return rmSingle (p);
		})));
};

// list meta values at given `path`
const lsmeta = path => safeExec (escapeValues`${KDB_COMMAND} lsmeta -0 ${path}`).then(stdout => stdout && stdout.split("\0"));

// get meta value from given `path`
const getmeta = (path, meta) => safeExec (escapeValues`${KDB_COMMAND} getmeta ${path} ${meta}`);

// set meta value at given `path`
const setmeta = (path, meta, value) => safeExec (escapeValues`${KDB_COMMAND} setmeta ${path} ${meta} ${value}`);

// remove meta value at given `path`
const rmmeta = (path, meta) => safeExec (escapeValues`${KDB_COMMAND} rmmeta ${path} ${meta}`);

// get all metavalues for given `path`
const getAllMeta = path => lsmeta (path)
				   .then(metaValues => metaValues && Promise.all(metaValues.map(meta => getmeta (path, meta).then(val => {
					   return { [meta]: val };
				   }))))
				   // merge objects
				   .then(resolvedMetaValues => resolvedMetaValues && Object.assign.apply(Object, resolvedMetaValues));
...
const _export = path => {
	...
	return safeExec (escapeValues`${KDB_COMMAND} export ${path} yajl ${buffer}`).then(() => {
		const data = JSON.parse(readFileSync (buffer));
		unlink (buffer, err => err && console.error("could not delete buffer file:", err));
		return data;
	});
};

// import javascript object at given `path`
const _import = (path, value) => safeExec (
