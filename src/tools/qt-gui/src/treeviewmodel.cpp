...
TreeViewModel::TreeViewModel(QObject* parent)
{
    ...
    try{
        ...
        emit showError("Could not read configuration.", "", QString(e.what()));
    }
}
...
QVariant TreeViewModel::data(const QModelIndex& index, int role) const
{
    if (!index.isValid())
    {
        emit showError("Index not valid.", QString("Index = " + index.row()) + QString("\nModel size = " + m_model.size()), "TreeViewModel::data");
        return QVariant();
    }

    if (index.row() > (m_model.size() - 1))
    {
        emit showError(QString("Index too high. "), "Index: " + index.row(), "TreeViewModel::data");
        return QVariant();
    }
    ...
    default:
        emit showError("Unknown role: " + role, "", "TreeViewModel::data");
        return QVariant();
...
void TreeViewModel::setData(int index, const QVariant& value, const QString& role)
{
    ...
    {
        emit showError("Index not valid.", QString("Index = " + index) + QString("\nModel size = " + m_model.size()), "TreeViewModel::setData");
        return;
    }
...
void TreeViewModel::deletePath(const QString &path)
{
    ...
    {
        emit showError("Invalid path.", "", "TreeViewModel::deletePath");
    }
}
...
void TreeViewModel::importConfiguration(const QString &name, const QString &format, QString &file, const QString &mergeStrategy)
{
    ...
    {
        emit showError("Importing the configuration from file terminated unsuccessfully because of a faulty command.", "", QString(ce.what()));
    }
    catch (kdb::Key& key)
    {
        stringstream ws;
    ...
    {
        emit showError("Importing the configuration from file terminated unsuccessfully.", "", QString(ce.what()));
    }
    catch (...)
    {
        emit showError("Unknown error", "", "TreeViewModel::importConfiguration");
    }
...
void TreeViewModel::exportConfiguration(ConfigNode *node, QString format, QString file)
{
    ...
    {
        emit showError("Exporting the configuration to file terminated unsuccessfully.", "", QString(ce.what()));
        return;
    }
    catch (kdb::Key& key)
    {
    ...
    {
        emit showError("Unknown error", "", "TreeViewModel::exportConfiguration");
    }
}
...
bool TreeViewModel::removeRow(int row, const QModelIndex& parent)
{
    ...
    {
        emit showError("Index not valid.", QString("Model size = " +  m_model.size()) + QString("\nIndex = " + row), "TreeViewModel::removeRow");
        return false;
    }
...
void TreeViewModel::insertMetaRow(int row, ConfigNode *node)
{
    ...
    if(m_metaModelParent){
        ...
        emit showError("Inserting metakey failed.", "Key " + QString::fromStdString(node->getKey().getFullName()) + " is not valid.", "");
}
...
void TreeViewModel::createNewNode(const QString &path, const QString &value, const QVariantMap metaData)
{
    ...
    {
        emit showError(QString("Creating a new node failed because the key " + path) + " is invalid", "", "TreeViewModel::createNewNode");
    }
}
...
void TreeViewModel::synchronize()
{
    ...
    try{
        ...
        emit showError("Synchronizing failed.", "", QString(e.what()));
    }
}
