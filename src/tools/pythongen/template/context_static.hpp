...
directiveStartToken = @cheetahVarStartToken = $
					      ...
					      @from support.context import * @from util import util @from cpp_util import cpp_util
					      @set support = ContextSupport () $util.header ($args.output)
...
public:
	...
	@ if $support.typeof ($hierarchy.info) == "kdb::none_t" static kdb::Key get (kdb::KeySet &, kdb::Key const &)
	{
		none_t none;
		return none;
	}
	@ else static kdb::Key get (kdb::KeySet & ks, kdb::Key const &)
	{
		type value
			$support.valof ($hierarchy.info)

				$cpp_util.generateGetBySpec (support, $hierarchy.name, hierarchy.info)
