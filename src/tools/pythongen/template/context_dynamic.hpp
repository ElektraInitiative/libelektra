...
public:
	...
	@ if $support.typeof ($hierarchy.info) == "kdb::none_t" static type get (kdb::KeySet &, kdb::Key const &)
	{
		none_t none;
		return none;
	}
	@ else static type get (kdb::KeySet & ks, kdb::Key const & spec)
	{
		type value $support.valof ($hierarchy.info)

			Key found (ckdb::ksLookup (ks.getKeySet (), *spec, ckdb::elektraLookupOptions::KDB_O_SPEC));
