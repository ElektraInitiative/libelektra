...
#include ELEKTRA_README
	...
	if (!repo)
	{
		ELEKTRA_SET_ERRORF (RESOURCE_CODE, parentKey, "Failed to open Repository %s\n", data->repo);
	...
	if (!headRef)
	{
		ELEKTRA_SET_ERRORF (RESOURCE_CODE, parentKey, "Failed to get reference %s\n", data->refName);
	...
	if (data->pull)
	{
		...
		if (rc)
		{
			ELEKTRA_SET_ERROR (CONFLICT_CODE, parentKey, "Fast-forward pull failed, please pull manually\n");
			git_repository_free (repo);
			git_libgit2_shutdown ();
			return -1;
		}
	...
	if (!blob)
	{
		ELEKTRA_ADD_WARNINGF (RESOURCE_CODE, parentKey, "File %s not found in repository %s\n", data->file, data->repo);
		git_repository_free (repo);
		git_libgit2_shutdown ();
		return 0;
	}
	...
	if (!outFile)
	{
		ELEKTRA_SET_ERRORF (RESOURCE_CODE, parentKey, "Failed to check out file %s to %s\n", data->file, keyString (parentKey));
		git_object_free (blob);
		git_repository_free (repo);
		git_libgit2_shutdown ();
		return -1;
...
int elektraGitresolverSet (Plugin * handle ELEKTRA_UNUSED, KeySet * returned ELEKTRA_UNUSED, Key * parentKey ELEKTRA_UNUSED)
{
	...
	if (data->tracking == HEAD)
	{
		...
		if (newCommit)
		{
			...
			ELEKTRA_SET_ERROR (CONFLICT_CODE, parentKey, "The repository has been updated and is ahead of you");
