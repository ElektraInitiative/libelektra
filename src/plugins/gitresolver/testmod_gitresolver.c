 ...
 * @file
 *
 * @brief Tests for gitresolver plugin
 *
...
static void test_basics ()
{
	...
	Key * parentKey = keyNew ("user/tests/gitresolver", KEY_END);
	KeySet * conf = ksNew (0, KS_END);
	PLUGIN_OPEN ("gitresolver");
...
int main (int argc, char ** argv)
{
	...
	printf ("\ntestmod_gitresolver RESULTS: %d test(s) done. %d error(s).\n", nbTest, nbError);
