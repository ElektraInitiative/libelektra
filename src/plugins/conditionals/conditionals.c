...
static const char * isAssign (Key * key, char * expr, Key * parentKey, KeySet * ks)
{
	...
	if (*firstPtr != '\'' || *lastPtr != '\'')
	{
		if (lastPtr <= firstPtr)
		{
			ELEKTRA_SET_ERRORF (ELEKTRA_ERROR_VALIDATION_SYNTACTIC, parentKey,
		...
		*(lastPtr + 1) = '\0';
		...
		if (!assign)
		{
			ELEKTRA_SET_ERRORF (ELEKTRA_ERROR_VALIDATION_SEMANTIC, parentKey, "Key %s not found", keyName (lookupKey));
			keyDel (lookupKey);
			return NULL;
		}
		else
	...
	else if (ret == FALSE)
	{
		if (elseexpr)
		{
			...
			else
			{
				...
				{
					ELEKTRA_SET_ERRORF (
						ELEKTRA_ERROR_VALIDATION_SYNTACTIC, parentKey,
