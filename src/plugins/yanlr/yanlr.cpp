...
int parseYAML (ifstream & file, CppKeySet & keys, CppKey & parent)
{
	...
	{
		ELEKTRA_SET_ERROR (ELEKTRA_ERROR_PARSING, parent.getKey (), errorListener.message ());
...
int elektraYanlrGet (Plugin * handle ELEKTRA_UNUSED, KeySet * returned, Key * parentKey)
{
	...
	if (!file.is_open ())
	{
		ELEKTRA_SET_ERRORF (ELEKTRA_ERROR_RESOURCE, parent.getKey (), "Unable to open file “%s”", parent.getString ().c_str ());
		return ELEKTRA_PLUGIN_STATUS_ERROR;
	}
