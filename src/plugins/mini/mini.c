...
static inline void parseLine (char * line, size_t lineNumber, KeySet * keySet, Key * parentKey)
{
	...
	if (*equals == '\0' || equals == pair)
	{
		...
		ELEKTRA_ADD_PARSING_WARNINGF (parentKey, "Line %zu: “%s” is not a valid key value pair", lineNumber, pair);
		return;
	}
...
static int parseINI (FILE * file, KeySet * keySet, Key * parentKey)
{
	...
	if (!feof (file))
	{
		...
		ELEKTRA_SET_PARSING_ERRORF (parentKey, "Unable to read line %zu: %s", lineNumber, strerror (errno));
		errno = errorNumber;
