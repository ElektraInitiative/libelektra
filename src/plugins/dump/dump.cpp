...
int unserialise (std::istream & is, ckdb::Key * errorKey, ckdb::KeySet * ks)
{
	...
	while (std::getline (is, line))
	{
		...
		if (command == "kdbOpen")
		{
			...
			if (version != "1")
			{
				ELEKTRA_SET_PARSING_ERRORF (errorKey, "Wrong version detected in dumpfile: %s", version.c_str ());
				return -1;
			}
		}
		...
		{
			ELEKTRA_SET_PARSING_ERRORF (errorKey, "Wrong version detected in dumpfile: %s", command.c_str ());
			return -1;
		}
	}
	return 1;
