...
int PYTHON_PLUGIN_FUNCTION (Open) (ckdb::Plugin * handle, ckdb::Key * errorKey)
{
	...
	if (pp == nullptr)
	{
		...
		if (!md)
		{
			...
			ELEKTRA_SET_ERROR (ELEKTRA_ERROR_INSTALLATION, errorKey, "No python script set, please pass a filename via /script");
	...
	if (data->instance == nullptr)
	{
		...
		{
			ELEKTRA_SET_ERRORF (ELEKTRA_ERROR_INSTALLATION, errorKey, "Unable to extend sys.path with user-defined /python/path \"%s\"",
					    mname);
			goto error;
		}
		...
		{
			ELEKTRA_SET_ERRORF (ELEKTRA_ERROR_INSTALLATION, errorKey, "Unable to extend sys.path with built-in plugin path \"%s\"",
		...
		{
			ELEKTRA_SET_ERRORF (ELEKTRA_ERROR_INSTALLATION, errorKey, "Unable to extend sys.path with script dirname \"%s\"", dname);
		...
		if (pModule == nullptr)
		{
			ELEKTRA_SET_ERRORF (ELEKTRA_ERROR_INSTALLATION, errorKey, "Unable to import python script \"%s\"", keyString (data->script));
			elektraFree (tmpScript);
			goto error_print;
		}
		elektraFree (tmpScript);
