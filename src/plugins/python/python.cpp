...
static int Python_CallFunction_Int (moduleData * data, PyObject * object, PyObject * args, ckdb::Key * errorKey)
{
	...
	if (!res)
	{
		ELEKTRA_SET_ERRORF (INSTALLATION_CODE, errorKey, "Error while calling python function of script %s%s",
	...
	else
	{
		...
		if (!PyInt_Check (res))
#endif
			ELEKTRA_SET_ERROR (VALIDATION_SEMANTIC_CODE, errorKey, "Return value is no integer");
		else
...
int PYTHON_PLUGIN_FUNCTION (Open) (ckdb::Plugin * handle, ckdb::Key * errorKey)
{
	...
	if (pp == nullptr)
	{
		...
		if (!md)
		{
			...
			ELEKTRA_SET_ERROR (INSTALLATION_CODE, errorKey, "No python script set, please pass a filename via /script");
	...
	if (data->instance == nullptr)
	{
		...
		{
			ELEKTRA_SET_ERROR (INSTALLATION_CODE, errorKey, "Unable to create sub interpreter");
			goto error;
		}
		...
		{
			ELEKTRA_SET_ERRORF (INSTALLATION_CODE, errorKey, "Unable to extend sys.path with built-in path \"%s\"",
		...
		{
			ELEKTRA_SET_ERRORF (INSTALLATION_CODE, errorKey, "Unable to extend sys.path with user-defined /python/path \"%s\"",
					    mname);
			goto error;
		}
		...
		{
			ELEKTRA_SET_ERROR (INSTALLATION_CODE, errorKey, "Unable to import kdb module");
			goto error_print;
		}
		...
		{
			ELEKTRA_SET_ERRORF (INSTALLATION_CODE, errorKey, "Unable to extend sys.path with built-in plugin path \"%s\"",
		...
		{
			ELEKTRA_SET_ERRORF (INSTALLATION_CODE, errorKey, "Unable to extend sys.path with script dirname \"%s\"", dname);
		...
		if (pModule == nullptr)
		{
			ELEKTRA_SET_ERRORF (INSTALLATION_CODE, errorKey, "Unable to import python script \"%s\"", keyString (data->script));
			elektraFree (tmpScript);
			goto error_print;
		}
		elektraFree (tmpScript);
		...
		if (klass == nullptr)
		{
			ELEKTRA_SET_ERROR (INSTALLATION_CODE, errorKey, "Module doesn't provide a ElektraPlugin class");
			goto error_print;
		}
		...
		if (inst == nullptr)
		{
			ELEKTRA_SET_ERROR (INSTALLATION_CODE, errorKey, "Unable to create instance of ElektraPlugin");
			goto error_print;
		}
		data->instance = inst;
	}
