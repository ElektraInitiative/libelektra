...
static void test_basics (void)
{
	...
	ElektraKey * parentKey = elektraKeyNew ("user:/tests/process", ELEKTRA_KEY_VALUE, tmpFile, ELEKTRA_KEY_END);
	...
	ElektraKey * pluginKey = elektraKeyNew ("/plugin", ELEKTRA_KEY_VALUE, "dump", ELEKTRA_KEY_END);
	elektraKeysetAppendKey (conf, pluginKey);
	PLUGIN_OPEN ("process");
	...
	ElektraKey * contractKey = elektraKeyNew ("system:/elektra/modules/process", ELEKTRA_KEY_END);
	ElektraKeyset * contractSet = elektraKeysetNew (0, ELEKTRA_KS_END);
	...
	elektraKeyDel (contractKey);
	elektraKeysetDel (contractSet);
...
static void test_no_plugin_key (void)
{
	...
	ElektraKey * parentKey = elektraKeyNew ("user:/tests/process", ELEKTRA_KEY_END);
	ElektraKeyset * conf = elektraKeysetNew (0, ELEKTRA_KS_END);
...
static void test_invalid_plugin_key (void)
{
	...
	ElektraKey * parentKey = elektraKeyNew ("user:/tests/process", ELEKTRA_KEY_END);
	ElektraKeyset * conf = elektraKeysetNew (0, ELEKTRA_KS_END);
	ElektraKey * pluginKey = elektraKeyNew ("/plugin", ELEKTRA_KEY_VALUE, "non_existent_plugin", ELEKTRA_KEY_END);
	elektraKeysetAppendKey (conf, pluginKey);
