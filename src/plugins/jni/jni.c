...
static void checkException(Data *data, const char *when, Key *warningKey)
{
	if ((*data->env)->ExceptionCheck(data->env))
	{
		...
		jmethodID toString = (*data->env)->GetMethodID(
			data->env,
			(*data->env)->FindClass(data->env, "java/lang/Object"),
			"toString",
			"()Ljava/lang/String;");
		jstring estr = (jstring) (*data->env)->CallObjectMethod(
			data->env,
			ex, toString);
...
static int call1Arg(Data *data, Key *errorKey, const char *method)
{
	...
	if (jerrorKey == 0)
	{
		ELEKTRA_SET_ERRORF(102,
			errorKey,
			"Cannot create errorKey in %s",
			method);
		return -1;
	}
...
int elektraJniOpen(Plugin *handle, Key *errorKey)
{
	...
	*/
	...
	{
		ELEKTRA_SET_ERROR(102, errorKey,
				"Cannot find constructor of Key");
		return -1;
	}

	data->midKeySetConstr = (*data->env)->GetMethodID(data->env,
			data->clsKeySet,
			"<init>", "(J)V");
	if (data->midKeySetConstr == 0)
	{
		ELEKTRA_SET_ERROR(102, errorKey,
				"Cannot find constructor of KeySet");
		return -1;
	}
	...
	{
		ELEKTRA_SET_ERROR(102, errorKey,
				"Cannot find release of Key");
		return -1;
	}

	data->midKeySetRelease = (*data->env)->GetMethodID(data->env,
			data->clsKeySet,
			"release", "()V");
	if (data->midKeySetRelease == 0)
	{
		ELEKTRA_SET_ERROR(102, errorKey,
				"Cannot find release of KeySet");
		return -1;
	}
	...
	{
		ELEKTRA_SET_ERROR(102, errorKey,
				"Cannot find constructor of plugin");
		return -1;
	}
