...
static int handleInvalidConflict (Key * parentKey, Key * key, OnConflict onConflict)
{
	...
	{
	case ERROR:
		ELEKTRA_SET_VALIDATION_SEMANTIC_ERRORF (parentKey, "Invalid key %s\n", keyName (key));
		ret = -1;
		break;
	case WARNING:
		ELEKTRA_ADD_VALIDATION_SEMANTIC_WARNINGF (parentKey, "Invalid key %s\n", keyName (key));
		break;
	case INFO:
	{
...
static int handleArrayConflict (Key * parentKey, Key * key, Key * conflictMeta, OnConflict onConflict)
{
	...
	{
	case ERROR:
		ELEKTRA_SET_VALIDATION_SEMANTIC_ERRORF (parentKey, "%s has invalid array key members: %s\n", keyName (key), problemKeys);
		ret = -1;
		break;
	case WARNING:
		ELEKTRA_ADD_VALIDATION_SEMANTIC_WARNINGF (parentKey, "%s has invalid array members: %s\n", keyName (key), problemKeys);
		break;
	case INFO:
	{
...
static int handleSubCountConflict (Key * parentKey, Key * key, Key * specKey, Key * conflictMeta, OnConflict onConflict)
{
	...
	{
	case ERROR:
		ELEKTRA_SET_VALIDATION_SEMANTIC_ERRORF (parentKey, "%s has an invalid number of subkeys: %s. Expected: %s\n", keyName (key),
							keyString (conflictMeta), keyString (keyGetMeta (specKey, "required")));
		ret = -1;
		break;
	case WARNING:
		ELEKTRA_ADD_VALIDATION_SEMANTIC_WARNINGF (parentKey, "%s has an invalid number of subkeys: %s. Expected: %s\n",
							  keyName (key), keyString (conflictMeta),
							  keyString (keyGetMeta (specKey, "required")));
		break;
	case INFO:
	{
...
static int handleOutOfRangeConflict (Key * parentKey, Key * key, Key * specKey, Key * conflictMeta, OnConflict onConflict)
{
	...
	{
	case ERROR:
		ELEKTRA_SET_VALIDATION_SEMANTIC_ERRORF (parentKey, "%s has invalid number of members: %s. Expected: %s\n", keyName (key),
							keyString (conflictMeta), keyString (keyGetMeta (specKey, "array")));
		ret = -1;
		break;
	case WARNING:
		ELEKTRA_ADD_VALIDATION_SEMANTIC_WARNINGF (parentKey, "%s has invalid number of members: %s. Expected: %s\n", keyName (key),
							  keyString (conflictMeta), keyString (keyGetMeta (specKey, "array")));
		break;
	case INFO:
	{
...
static int handleMissingConflict (Key * parentKey, Key * key, Key * conflictMeta, OnConflict onConflict)
{
	...
	{
	case ERROR:
		ELEKTRA_SET_VALIDATION_SEMANTIC_ERRORF (parentKey, "%s has missing subkeys: %s\n", keyName (key), problemKeys);
		ret = -1;
		break;
	case WARNING:
		ELEKTRA_ADD_VALIDATION_SEMANTIC_WARNINGF (parentKey, "%s has missing subkeys: %s\n", keyName (key), problemKeys);
		break;
	case INFO:
	{
