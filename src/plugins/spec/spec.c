...
typedef enum{ARRAYMEMBER, INVALID, SUBCOUNT, CONFLICT, OUTOFRANGE, MISSING, NAC}Conflict;
...
static int handleErrors(Key *parentKey, KeySet *ks, Key *key, Key *specKey, ConflictHandling *ch, Direction dir)
{
	...
	while(keyNextMeta(parent) != NULL)
	{
		...
		else if(!strncmp(keyName(meta), "conflict/#", 10) || !strncmp(keyName(meta), "conflict/hasInvalidMembers/#", 28))
		{
			keySetMeta(parent, keyName(meta), 0);
		}
	}
...
static void copyMeta(Key *key, Key *specKey)
{
	...
	while(keyNextMeta(specKey) != NULL)
	{
		...
		if(!(!strcmp(name, "array") || !strcmp(name, "required") || !strncmp(name, "conflict/", 9) || !strcmp(name, "require")))
		{
			const Key *oldMeta;
