...
static int checkLineEndings (const char * fileName, Lineending validLineEnding, Key * parentKey)
{
	...
	while (!feof (fp))
	{
		...
		if (lineEnding == NA && found != NA)
		{
			...
			{
				fclose (fp);
				ELEKTRA_SET_ERRORF (ELEKTRA_ERROR_PARSING, parentKey, "Invalid line ending at line %lu", line);
				return -2;
			}
			++line;
		}
		...
		{
			fclose (fp);
			ELEKTRA_SET_ERRORF (ELEKTRA_ERROR_PARSING, parentKey, "inconsistent line endings at line %lu", line);
			return -3;
		}
		fc = sc;
		found = NA;
...
int elektraLineendingsSet (Plugin * handle, KeySet * returned ELEKTRA_UNUSED, Key * parentKey)
{
	...
	{
	case (-1):
		ELEKTRA_SET_ERRORF (ELEKTRA_ERROR_RESOURCE, parentKey, "Couldn't open file %s\n", keyString (parentKey));
		return 1;
		break;
	case (-2):
		return -1;
