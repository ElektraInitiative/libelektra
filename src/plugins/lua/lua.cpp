...
static int Lua_CallFunction_Int (lua_State * L, int nargs, ckdb::Key * errorKey)
{
	...
	if (lua_pcall (L, nargs, 1, 0) != LUA_OK)
		ELEKTRA_SET_ERROR (VALIDATION_SEMANTIC_CODE, errorKey, lua_tostring (L, -1));
	else
	{
		if (!lua_isnumber (L, -1))
...
int elektraLuaOpen (ckdb::Plugin * handle, ckdb::Key * errorKey)
{
	...
	if (script == NULL || !strlen (keyString (script)))
	{
		...
		ELEKTRA_SET_ERROR (INSTALLATION_CODE, errorKey, "No lua script set");
		return -1;
	}
	...
	{
		ELEKTRA_SET_ERROR (INSTALLATION_CODE, errorKey, "Unable to create new lua state");
		goto error;
	}
...
error_print:
	if (!lua_isnil (data->L, -1)) ELEKTRA_SET_ERROR (INSTALLATION_CODE, errorKey, lua_tostring (data->L, -1));
error:
	/* destroy lua */
	Lua_Shutdown (data->L);
	delete data;
