...
static gpgme_key_t * extractRecipientFromPluginConfig (KeySet * config, Key * errorKey, gpgme_ctx_t ctx)
{
	...
	if (gpgRecipientRoot && strlen (keyString (gpgRecipientRoot)) > 0)
	{
		...
		if (err)
		{
			ELEKTRA_SET_VALIDATION_SEMANTIC_ERROR (errorKey, "Failed to read the specified GPG key. Reason: %s", gpgme_strerror (err));
	...
	if (gpgRecipientRoot)
	{
		...
		while ((k = ksNext (config)) != 0)
		{
			if (keyIsBelow (k, gpgRecipientRoot))
			{
				...
				if (err)
				{
					ELEKTRA_SET_VALIDATION_SEMANTIC_ERROR (errorKey, "Failed to read the specified GPG key. Reason: %s",
								     gpgme_strerror (err));
					elektraGpgmeKeylistFree (&list);
					return NULL;
				}
