...
static int transferGpgmeDataToElektraKey (gpgme_data_t src, Key * dst, Key * errorKey, int textMode)
{
	...
	{
		ELEKTRA_SET_ERROR (ELEKTRA_ERROR_LOGICAL, errorKey, "An error during occurred during the data transfer.");
		returnValue = -1; // failure
		goto cleanup;
	}
...
static int gpgEncrypt (Plugin * handle, KeySet * data, Key * errorKey)
{
	...
	if (!recipients)
	{
		ELEKTRA_SET_ERROR (ELEKTRA_ERROR_VALIDATION_SEMANTIC, errorKey, "No valid recipients were specified.");
		returnValue = -1;
		goto cleanup;
	}
	...
	while ((k = ksNext (data)))
	{
		...
		{
			returnValue = -1;
			ELEKTRA_SET_ERRORF (ELEKTRA_ERROR_LOGICAL, errorKey, "Internal error: %s", gpgme_strerror (err));
			goto cleanup;
		}
...
static int gpgDecrypt (ELEKTRA_UNUSED Plugin * handle, KeySet * data, Key * errorKey)
{
	...
	if (err)
	{
		ELEKTRA_SET_ERRORF (ELEKTRA_ERROR_INSTALLATION, errorKey, "Failed to the gpgme context because: %s", gpgme_strerror (err));
		return -1; // at this point nothing has been initialized
	}
