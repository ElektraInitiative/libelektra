...
static gpgme_key_t * extractRecipientFromPluginConfig (KeySet * config, Key * errorKey, gpgme_ctx_t ctx)
{
	...
	if (gpgRecipientRoot && strlen (keyString (gpgRecipientRoot)) > 0)
	{
		...
		if (err)
		{
			ELEKTRA_SET_ERRORF (LOGICAL_CODE, errorKey, "Failed to receive the GPG key because: %s", gpgme_strerror (err));
	...
	if (gpgRecipientRoot)
	{
		...
		while ((k = ksNext (config)) != 0)
		{
			if (keyIsBelow (k, gpgRecipientRoot))
			{
				...
				if (err)
				{
					ELEKTRA_SET_ERRORF (LOGICAL_CODE, errorKey, "Failed to receive the GPG key because: %s",
							    gpgme_strerror (err));
					elektraGpgmeKeylistFree (&list);
					return NULL;
				}
...
static int transferGpgmeDataToElektraKey (gpgme_data_t src, Key * dst, Key * errorKey, int textMode)
{
	...
	{
		ELEKTRA_SET_ERROR (LOGICAL_CODE, errorKey, "An error during occurred during the data transfer.");
		returnValue = -1; // failure
		goto cleanup;
	}
...
static int gpgEncrypt (Plugin * handle, KeySet * data, Key * errorKey)
{
	...
	if (err)
	{
		ELEKTRA_SET_ERRORF (INSTALLATION_CODE, errorKey, "Failed to create the gpgme context because: %s", gpgme_strerror (err));
	...
	if (!recipients)
	{
		ELEKTRA_SET_ERROR (VALIDATION_SEMANTIC_CODE, errorKey, "No valid recipients were specified.");
		returnValue = -1;
		goto cleanup;
	}
	...
	while ((k = ksNext (data)))
	{
		...
		{
			returnValue = -1;
			ELEKTRA_SET_ERRORF (LOGICAL_CODE, errorKey, "Internal error: %s", gpgme_strerror (err));
			goto cleanup;
		}
		...
		if (invalidKey)
		{
			...
			if (errorMsg)
			{
				ELEKTRA_SET_ERRORF (VALIDATION_SEMANTIC_CODE, errorKey, "Invalid key ID(s): %s", errorMsg);
...
static int gpgDecrypt (ELEKTRA_UNUSED Plugin * handle, KeySet * data, Key * errorKey)
{
	...
	if (err)
	{
		ELEKTRA_SET_ERRORF (INSTALLATION_CODE, errorKey, "Failed to the gpgme context because: %s", gpgme_strerror (err));
		return -1; // at this point nothing has been initialized
	}
