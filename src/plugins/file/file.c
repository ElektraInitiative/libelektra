...
#include ELEKTRA_README
	...
	{
		ELEKTRA_SET_GENERAL_RESOURCE_ERRORF (parentKey, "failed to stat file %s, aborting. Reason: %s", fileName, strerror (errno));
		return -1;
	}
	...
	if (!buffer)
	{
		ELEKTRA_SET_GENERAL_RESOURCE_ERRORF (parentKey, "failed to allocate buffer of %lld bytes for %s", fileSize, fileName);
		return -1;
	}
	...
	if (!fp)
	{
		ELEKTRA_SET_GENERAL_RESOURCE_ERRORF (parentKey, "Failed to open file %s", fileName);
		elektraFree (buffer);
		return -1;
	}
...
int elektraFileSet (Plugin * handle ELEKTRA_UNUSED, KeySet * returned ELEKTRA_UNUSED, Key * parentKey ELEKTRA_UNUSED)
{
	...
	if (!fp)
	{
		ELEKTRA_SET_GENERAL_RESOURCE_ERRORF (parentKey, "failed to open %s for writing", fileName);
		return -1;
	}
