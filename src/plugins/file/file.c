...
#include ELEKTRA_README (file)
	...
	{
	    ELEKTRA_SET_ERRORF(ELEKTRA_ERROR_STAT_FAILED, parentKey, "failed to stat file %s, aborting. Reason: %s", fileName, strerror(errno));
	    return -1;
	}
	...
	if(!buffer)
	{
	    ELEKTRA_SET_ERRORF(ELEKTRA_ERROR_MALLOC_ERROR, parentKey, "failed to allocate buffer of %lld bytes for %s", fileSize, fileName);
	    return -1;
	}
	...
	{
	    ELEKTRA_SET_ERRORF(ELEKTRA_ERROR_READ_FAILED, parentKey, "failed to read %s completely. got %lld of %lld bytes", fileName, bytesRead, fileSize);
	    elektraFree(buffer);
	    fclose(fp);
	    return -1;
	}
