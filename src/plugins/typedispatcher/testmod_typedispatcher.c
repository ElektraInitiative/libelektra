...
void testSubFail ()
{
	...
	KeySet * conf = ksNew (3, keyNew ("system/error", KEY_VALUE, "FAIL", KEY_END), KS_END);
	PLUGIN_OPEN ("typedispatcher");
	succeed_if (plugin->kdbGet (plugin, ks, parentKey) == -1, "SubFail failed");
	ksDel (ks);
	keyDel (parentKey);
	PLUGIN_CLOSE ();
}
...
void testMultiSub ()
{
	...
	//	fprintf (stderr, "\t=================\n");
	ksAppendKey (ks, keyNew ("user/tests/typedispatcher/sub/sub/Ckey", KEY_VALUE, "C", KEY_META, "type", "multi", KEY_END));
...
void testMultiParametersRecursive ()
{
	...
	succeed_if (plugin->kdbGet (plugin, ks, parentKey) == 1, "MultiParameters Ext 1 failed");
	//	fprintf (stderr, "\t=================\n");
	ksAppendKey (ks, keyNew ("user/tests/typedispatcher/sub/sub/Ckey", KEY_VALUE, "C", KEY_META, "type", "multi (A,B)", KEY_END));
	succeed_if (plugin->kdbGet (plugin, ks, parentKey) == -1, "MultiParameters Ext 2 failed");
	ksDel (ks);
	keyDel (parentKey);
	PLUGIN_CLOSE ();
...
void testSubFailDrop ()
{
	...
	PLUGIN_OPEN ("typedispatcher");
	succeed_if (plugin->kdbGet (plugin, ks, parentKey) == 1, "SubFailIgnore failed");
	Key * lookup = ksLookupByName (ks, "user/tests/typedispatcher/sub/ab", KDB_O_NONE);
	succeed_if (!lookup, "key not dropped");
	ksDel (ks);
	keyDel (parentKey);
	PLUGIN_CLOSE ();
}
...
void testSubFailDefault ()
{
	...
	PLUGIN_OPEN ("typedispatcher");
	succeed_if (plugin->kdbGet (plugin, ks, parentKey) == 1, "SubFailIgnore failed");
	Key * lookup = ksLookupByName (ks, "/tests/typedispatcher/sub/ab", KDB_O_NONE);
	succeed_if (lookup, "default key not found");
	succeed_if (!strcmp (keyString (lookup), "defaultKey"), "not expected default key");
	ksDel (ks);
	keyDel (parentKey);
	PLUGIN_CLOSE ();
}
...
int main (int argc, char ** argv)
{
	...
	//	fprintf (stderr, "\n============================================\n");
	//	fprintf (stderr, "testSubFailIgnore()\n");
	testSubFailIgnore ();
	//	fprintf (stderr, "\n============================================\n");
	//	fprintf (stderr, "testSubFailDrop()\n");
	testSubFailDrop ();
	//	fprintf (stderr, "\n============================================\n");
