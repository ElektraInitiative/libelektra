				...
				if (*endPtr == '%')
				{
					...
					*dstPtr = '\0';
					...
					if (lookup)
					{
						...
						ssize_t newLen = preparedStringLen + elektraStrLen (keyString (lookup)) -
								 1; // no need for 2 nullbytes
...
#ifdef DEVBUILD
			fprintf (stderr, "mapping argument %s:(%s) to parameter %s:(%s)\n", keyName (argKey), keyString (argKey),
				 keyName (paramKey), keyString (paramKey));
#endif
...
void parseMetadata (const Key * key, KeySet * ks, KeySet * conf)
{
	Key * cutKey = keyNew (keyName (key), KEY_END);
	keyAddBaseName (cutKey, "check");
	KeySet * cutKS = ksCut (ks, cutKey);
	Key * cur = NULL;
	ksRewind (cutKS);
	while ((cur = ksNext (cutKS)) != NULL)
	{
		KeySet * subCutKS = ksCut (cutKS, cur);
		ksRewind (subCutKS);
		Key * usedBy = keyNew (keyName (cur), KEY_END);
		keyAddBaseName (usedBy, "usedby");
		keyAddBaseName (usedBy, "plugin");
		Key * lookup = ksLookup (subCutKS, usedBy, KDB_O_NONE);
		if (!lookup)
		{
			keyDel (usedBy);
			ksDel (subCutKS);
		...
		{
			const char * relName = elektraKeyGetRelativeName (cur, cutKey);
			Key * meta = keyNew (relName, KEY_VALUE, keyString (lookup), KEY_META_NAME, KEY_END);
			ksAppendKey (conf, meta);
		}
		keyDel (usedBy);
		ksDel (subCutKS);
	}
