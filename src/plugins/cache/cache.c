...
static int resolveCacheDirectory (Plugin * handle, CacheHandle * ch, Key * errorKey)
{
	...
	if (!ch->resolver)
	{
		ELEKTRA_ADD_INSTALLATION_WARNINGF (errorKey, "Open of plugin returned unsuccessfully: %s",KDB_RESOLVER);
...
static int loadCacheStoragePlugin (Plugin * handle, CacheHandle * ch, Key * errorKey)
{
	...
	if (!ch->cacheStorage)
	{
		ELEKTRA_ADD_INSTALLATION_WARNINGF (errorKey, "Open of plugin returned unsuccessfully: %s", KDB_CACHE_STORAGE);
