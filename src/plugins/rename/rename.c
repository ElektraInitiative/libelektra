...
Key * elektraKeyCreateNewName (const Key * key, const Key * parentKey, const char * cutPath, const char * replaceWith, const char * toUpperPath, const char * toLowerPath, const int initialConversion)
{
    size_t addToLen = 0;
    if(replaceWith != NULL)
...
static Key * renameGet(Key * key, Key * parentKey, Key * cutConfig, Key * replaceWithConfig, Key * toUpperConfig, Key * toLowerConfig, Key *getCase)
{
    char *cutPath = 0;
    char *replaceWith = 0;
    char *toUpperPath = 0;
...
int elektraRenameSet (Plugin * handle, KeySet * returned, Key * parentKey)
{
    ...
    fprintf(stderr, "writeConversion: %d\n", writeConversion);
    ksRewind(iterateKs);
    Key *key;
    char *parentKeyName = elektraMalloc(keyGetFullNameSize(parentKey));
    keyGetFullName(parentKey, parentKeyName, keyGetFullNameSize(parentKey));
    ...
    while((key = ksNext(iterateKs)) != 0)
    {
        ...
        if(writeConversion != KEYNAME)
        {
             ...
             */
            fprintf(stderr, "renamedKey: %s\t key: %s\n", keyName(renamedKey), keyName(key));
            if (keyCmp(key, parentKey) != 0)
            {
                Key *tmp;
                tmp = ksLookup(returned, key, KDB_O_POP);
