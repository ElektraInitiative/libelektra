...
#include "readme_simpleini.c"
	...
	if (!format)
	{
		ELEKTRA_SET_ERROR (ELEKTRA_ERROR_PARSING, parentKey, "invalid 'format' specified");
		return -1;
	}
...
#pragma GCC diagnostic ignored "-Wformat"
	...
	while ((n = fscanf (fp, format, &key, &value)) >= 0)
	{
		...
		if (n == 0)
		{
			...
			{
				ELEKTRA_SET_ERRORF (ELEKTRA_ERROR_PARSING, parentKey, "failed discarding rest of line at position %ld with key %s",
						    ftell (fp), key);
				elektraFree (key);
				fclose (fp);
				return -1;
		...
		{
			ELEKTRA_SET_ERRORF (ELEKTRA_ERROR_PARSING, parentKey, "duplicated key %s at position %ld", keyName (read), ftell (fp));
			elektraFree (format);
			fclose (fp);
			return -1;
		}
	...
	if (feof (fp) == 0)
	{
		ELEKTRA_SET_ERRORF (ELEKTRA_ERROR_PARSING, parentKey, "not at the end of file at position %ld", ftell (fp));
		elektraFree (format);
		fclose (fp);
		return -1;
	}
