...
static int isoStringValidation(const char *date, const char *isoString)
{
    ...
    int hasTime = 0;
    int hasTZ = 0;
    if(strstr(isoString, "<date>") || strstr(isoString, "<calendardate>") || strstr(isoString, "<weekdate>") || strstr(isoString, "<ordinaldate>"))
	hasDate = 1;
    if(strchr(isoString, 'T'))
	hasT = 1;
    if(strstr(isoString, "<time>"))
	hasTime = 1;
    if(strstr(isoString, "<tz>"))
	hasTZ = 1;
    if(!hasT && ((hasDate && hasTime) || (hasDate && hasTZ)))
	return 0;
    if(hasT && (!hasDate || !hasTime))
	return 0;
    struct tm tm;
    if(hasDate)
    {
...
static int validateDate(Key *key, Key *parentKey ELEKTRA_UNUSED)
{
	...
	else if(isoMeta)
	{
	    ...
	    if(rc == -1)
	    {
		ELEKTRA_SET_ERRORF(ELEKTRA_ERROR_D_T_FMT, parentKey, "%s doesn't match iso specification %s", date, keyString(isoMeta));
	    }
	}
	else
	{
	    ...
	    if(rc == -1)
	    {
		ELEKTRA_SET_ERRORF(ELEKTRA_ERROR_D_T_FMT, parentKey, "%s doesn't match environment format string %s", date, formatString);
	    }
	}
	return rc;
}
