...
static int validateKey (Key * key, Key * parentKey)
{
	...
	{
		ELEKTRA_SET_ERRORF (VALIDATION_SEMANTIC_CODE, parentKey, "Given path \"%s\" is not absolute", keyString (key));
		return 0;
	}
	int errnosave = errno;
	...
	if (stat (keyString (key), &buf) == -1)
	{
		...
		ELEKTRA_ADD_WARNINGF (RESOURCE_CODE, parentKey, "Could not stat file, message: %s", errmsg);
		elektraFree (errmsg);
		errno = errnosave;
		return -1;
	}
	else if (!strcmp (keyString (meta), "device"))
	{
		...
		{
			ELEKTRA_ADD_WARNINGF (RESOURCE_CODE, parentKey, "Device not found: %s", keyString (key));
		}
	}
	else if (!strcmp (keyString (meta), "directory"))
	{
		...
		{
			ELEKTRA_ADD_WARNINGF (RESOURCE_CODE, parentKey, "Directory not found: %s", keyString (key));
		}
	}
	return 1;
}
...
static int validatePermission (Key * key, Key * parentKey)
{
	...
	if (canAccess != 0)
	{
		ELEKTRA_SET_ERRORF (RESOURCE_CODE, parentKey, "User %s does not have required permission (%s) on %s", name, modes,
				    validPath);
		return -1;
	}
...
static int handleNoUserCase (Key * parentKey, const char * validPath, const char * modes)
{
	...
	if (result != 0)
	{
		ELEKTRA_SET_ERRORF (RESOURCE_CODE, parentKey, "User %s does not have required permission (%s) on %s", p->pw_name, modes,
				    validPath);
		return -1;
	}
	return 1;
