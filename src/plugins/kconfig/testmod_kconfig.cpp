...
#include "kconfig/test_valid.h"
		...
		CppKey parent{ TEST_VALID_PREFIX };
		parent.setString ("kconfig/test_validrc");
		test_read_and_write (parent, keys);
	}
}
...
TEST (kconfig, invalid_read_tests)
{
	...
	test_read_fails (
		"multiple.locales[en][de]" //
	);
	test_read_fails (
		"[content after the group]name\n"
	);
	test_read_fails (
		"[content after the group name] \n"
	);
	test_read_fails (
		"content after[locale] textt" //
	);
	test_read_fails (
		"[invalid escape \\character]" //
	);
	test_read_fails (
		"invalid escape \\character in=key name" //
	);
	test_read_fails (
		"invalid escape=\\character in key value" //
	);
