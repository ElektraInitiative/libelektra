...
{
	ELEKTRA_SET_ERRORF (VALIDATION_SEMANTIC_CODE, errorKey, "The type '%s' failed to match for '%s' with string: %s", getTypeName (key),
...
bool elektraTypeValidateKey (Plugin * handle, Key * key, Key * errorKey)
{
	...
	if (type == NULL)
	{
		ELEKTRA_SET_ERRORF (VALIDATION_SEMANTIC_CODE, errorKey, "Unknown type '%s' for key '%s'", typeName, keyName (key));
		return false;
	}

	if (type->normalize != NULL && !type->normalize (handle, key))
	{
		ELEKTRA_SET_ERRORF (VALIDATION_SEMANTIC_CODE, errorKey, "The value '%s' of key %s could not be normalized (type is '%s')",
...
#include ELEKTRA_README
	...
	while ((cur = ksNext (returned)))
	{
		...
		if (type == NULL)
		{
			ELEKTRA_SET_ERRORF (VALIDATION_SEMANTIC_CODE, parentKey, "Unknown type '%s' for key '%s'", typeName, keyName (cur));
