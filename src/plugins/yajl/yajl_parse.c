...
int elektraYajlGet (Plugin * handle ELEKTRA_UNUSED, KeySet * returned, Key * parentKey)
{
	...
	while (!done)
	{
		...
		if (rd == 0)
		{
			if (!feof (fileHandle))
			{
				ELEKTRA_SET_ERRORF (ELEKTRA_ERROR_RESOURCE, parentKey, "Error while reading file: %s", keyString (parentKey));
				fclose (fileHandle);
				yajl_free (hand);
				return -1;
			}
		...
		if (test_status)
		{
			...
			ELEKTRA_SET_ERROR (ELEKTRA_ERROR_PARSING, parentKey, (char *) str);
