...
int ELEKTRA_PLUGIN_FUNCTION (getSaltFromMetakey) (ElektraKey * errorKey, ElektraKey * k, kdb_octet_t ** salt, kdb_unsigned_long_t * saltLen)
{
	...
	const ElektraKey * meta = elektraKeyGetMeta (k, ELEKTRA_CRYPTO_META_SALT);
	if (!meta)
	{
	...
	int result = ELEKTRA_PLUGIN_FUNCTION (base64Decode) (errorKey, elektraKeyString (meta), salt, &saltLenInternal);
	if (result == -1)
	{
		ELEKTRA_SET_VALIDATION_SEMANTIC_ERRORF (errorKey, "Salt was not stored Base64 encoded in key %s", elektraKeyName (k));
		return -1;
	}
	else if (result == -2)
	{
...
int ELEKTRA_PLUGIN_FUNCTION (getSaltFromPayload) (ElektraKey * errorKey, ElektraKey * k, kdb_octet_t ** salt, kdb_unsigned_long_t * saltLen)
{
	...
	const ssize_t payloadLen = elektraKeyGetValueSize (k) - ELEKTRA_CRYPTO_MAGIC_NUMBER_LEN;
...
{
	ElektraKey * master = elektraKeysetLookupByName (config, ELEKTRA_CRYPTO_PARAM_MASTER_PASSWORD, 0);
	if (!master)
	{
		ELEKTRA_SET_INSTALLATION_ERRORF (errorKey, "Missing %s in plugin configuration", ELEKTRA_CRYPTO_PARAM_MASTER_PASSWORD);
		return NULL;
	}
	ElektraKey * msg = elektraKeyDup (master, ELEKTRA_KEY_CP_ALL);
...
{
	ElektraKey * k = elektraKeysetLookupByName (config, ELEKTRA_CRYPTO_PARAM_ITERATION_COUNT, 0);
	if (k)
	{
		const kdb_unsigned_long_t iterations = strtoul (elektraKeyString (k), NULL, 10);
		if (iterations > 0)
		{
			return iterations;
		}
...
int ELEKTRA_PLUGIN_FUNCTION (gpgEncryptMasterPassword) (ElektraKeyset * conf, ElektraKey * errorKey, ElektraKey * msgKey)
{
	...
	ElektraKey * root = elektraKeysetLookupByName (conf, ELEKTRA_RECIPIENT_KEY, 0);

	// check root key crypto/key
	if (root && strlen (elektraKeyString (root)) > 0)
	{
		recipientCount++;
	}
	...
	if (root && strlen (elektraKeyString (root)) > 0)
	{
		...
		argv[i++] = (char *) elektraKeyString (root);
	}
	...
	while ((k = elektraKeysetNext (conf)) != 0)
	{
		...
		if (elektraKeyIsBelow (k, root) && strlen (kStringVal) > 0)
		{
			argv[i++] = "-r";
			// NOTE argv[] values will not be modified, so const can be discarded safely
	...
	int base64Result = ELEKTRA_PLUGIN_FUNCTION (base64Encode) (errorKey, elektraKeyValue (msgKey), elektraKeyGetValueSize (msgKey), &base64Encoded);
	if (base64Encoded)
	{
		elektraKeySetString (msgKey, base64Encoded);
...
int ELEKTRA_PLUGIN_FUNCTION (gpgDecryptMasterPassword) (ElektraKeyset * conf, ElektraKey * errorKey, ElektraKey * msgKey)
{
	...
	ELEKTRA_PLUGIN_FUNCTION (base64Decode) (errorKey, elektraKeyString (msgKey), &binaryData, &binaryDataLength);
	if (binaryData)
	{
		elektraKeySetBinary (msgKey, binaryData, binaryDataLength);
