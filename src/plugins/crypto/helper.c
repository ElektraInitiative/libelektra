...
int ELEKTRA_PLUGIN_FUNCTION (getSaltFromMetakey) (Key * errorKey, Key * k, kdb_octet_t ** salt, kdb_unsigned_long_t * saltLen)
{
	...
	if (result == -1)
	{
		ELEKTRA_SET_ERROR (ELEKTRA_ERROR_LOGICAL, errorKey, "Salt was not stored Base64 encoded.");
		return -1;
	}
	else if (result == -2)
	{
...
int ELEKTRA_PLUGIN_FUNCTION (getSaltFromPayload) (Key * errorKey, Key * k, kdb_octet_t ** salt, kdb_unsigned_long_t * saltLen)
{
	...
	{
		ELEKTRA_SET_ERRORF (ELEKTRA_ERROR_LOGICAL, errorKey, "restored salt has invalid length of %u (payload length is: %zu)",
