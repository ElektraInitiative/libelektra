...
static int isExecutable (const char * file, Key * errorKey)
{
	if (access (file, F_OK))
	{
		if (errorKey)
		{
			ELEKTRA_SET_ERRORF (ELEKTRA_ERROR_RESOURCE, errorKey, "gpg binary %s not found", file);
		}
		return -1;
	}
	...
	if (access (file, X_OK))
	{
		if (errorKey)
		{
			ELEKTRA_SET_ERRORF (ELEKTRA_ERROR_RESOURCE, errorKey, "gpg binary %s has no permission to execute", file);
		}
		return -2;
	}
...
static int verifyGpgKeysInConf (Key * root, KeySet * conf, Key * errorKey)
{
	...
	if (strlen (rootValue) > 0)
	{
		...
		{
			ELEKTRA_SET_ERRORF (ELEKTRA_ERROR_VALIDATION_SEMANTIC, errorKey, GPG_ERROR_INVALID_KEY, rootValue);
			return -1; // failure
		}
	}
	...
	while ((k = ksNext (conf)) != 0)
	{
		if (keyIsBelow (k, root))
		{
			...
			{
				ELEKTRA_SET_ERRORF (ELEKTRA_ERROR_VALIDATION_SEMANTIC, errorKey, GPG_ERROR_INVALID_KEY, childValue);
				return -1; // failure
			}
		}
	}
...
int ELEKTRA_PLUGIN_FUNCTION (gpgCall) (KeySet * conf, Key * errorKey, Key * msgKey, char * argv[], size_t argc)
{
	...
	if (pipe (pipe_stdin))
	{
		ELEKTRA_SET_ERROR (ELEKTRA_ERROR_INSTALLATION, errorKey, "Pipe initialization failed");
		elektraFree (argv[0]);
		return -1;
	}
	...
	case -1:
		// fork() failed
		ELEKTRA_SET_ERROR (ELEKTRA_ERROR_INSTALLATION, errorKey, "fork failed");
		closePipe (pipe_stdin);
		closePipe (pipe_stdout);
		closePipe (pipe_stderr);
		elektraFree (buffer);
	...
	if (msgKey && sendMessageSize > 0)
	{
		...
		{
			ELEKTRA_SET_ERROR (ELEKTRA_ERROR_INSTALLATION, errorKey, "The communication with the GPG process failed.");
			closePipe (pipe_stdin);
			closePipe (pipe_stdout);
			closePipe (pipe_stderr);
			elektraFree (buffer);
	...
	case 1:
		// bad signature
		ELEKTRA_SET_ERROR (ELEKTRA_ERROR_INSTALLATION, errorKey, "GPG reported a bad signature");
		break;

	case GPG_CALL_DUP_STDIN:
		ELEKTRA_SET_ERROR (ELEKTRA_ERROR_INSTALLATION, errorKey, "failed to redirect stdin.");
		break;

	case GPG_CALL_DUP_STDOUT:
		ELEKTRA_SET_ERROR (ELEKTRA_ERROR_INSTALLATION, errorKey, "failed to redirect stdout.");
		break;

	case GPG_CALL_DUP_STDERR:
		ELEKTRA_SET_ERROR (ELEKTRA_ERROR_INSTALLATION, errorKey, "failed to redirect stderr.");
		break;

	case GPG_CALL_EXECV:
		ELEKTRA_SET_ERRORF (ELEKTRA_ERROR_INSTALLATION, errorKey, "failed to start the gpg binary: %s", argv[0]);
		break;
	...
	default:
		...
		ELEKTRA_SET_ERRORF (ELEKTRA_ERROR_INSTALLATION, errorKey, "GPG failed with return value %d. %s", status, errorBuffer);
		break;
	}
