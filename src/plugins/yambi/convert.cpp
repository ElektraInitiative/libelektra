...
int addToKeySet (CppKeySet & keySet, CppKey & parent, string const & filename)
{
	...
	if (status < 0)
	{
		if (status == -3)
		{
			ELEKTRA_SET_ERRORF (ELEKTRA_ERROR_RESOURCE, parent.getKey (), "Unable to open file “%s”", filename.c_str ());
		}
		else if (status == -2)
		{
			ELEKTRA_SET_ERROR (ELEKTRA_ERROR_PARSING, parent.getKey (), "Parsing failed due to memory exhaustion");
		}
		else if (status == -1)
		{
			ELEKTRA_SET_ERROR (ELEKTRA_ERROR_PARSING, parent.getKey (), driver.getErrorMessage ().c_str ());
		}
