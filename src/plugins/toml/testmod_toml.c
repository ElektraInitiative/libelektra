...
#define TEST_RW_HEAD(t) const char * expectedType = t; size_t order = 0; KeySet * writeKs = ksNew(0, KS_END); KeySet * expectedKs = ksNew(0, KS_END)
#define TEST_RW_FOOT testWriteRead(writeKs, expectedKs); ksDel (expectedKs); ksDel (writeKs)
...
static void addKeyPair(KeySet * writeKs, KeySet * expectedKs, const char * name, const char * value, const char * origValue, const char * type, size_t order);
...
static Key * buildFullKey(const char * name, const char * value, const char * origValue, const char * type, size_t order) {
	...
	Key * key = keyNew (PREFIX,
		KEY_VALUE, value,
		KEY_META, "type", type,
		KEY_META, "order", orderStr,
	KEY_END);
	keyAddName(key, "/");
	keyAddName(key, name);
	if (origValue != NULL) {
...
static void testWriteRead(KeySet * ksWrite, KeySet * expected) {
	...
	succeed_if (plugin->kdbSet (plugin, ksWrite, parentKey) == ELEKTRA_PLUGIN_STATUS_SUCCESS,
		"Could not write keys");

	KeySet * ksRead = ksNew(0, KS_END);
	succeed_if (plugin->kdbGet (plugin, ksRead, parentKey) == ELEKTRA_PLUGIN_STATUS_SUCCESS,
		    "Could not read written keys");
