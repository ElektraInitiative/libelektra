...
static char * parseLine (char * origLine, char delim, unsigned long offset, Key * parentKey, unsigned long lineNr, int lastLine)
{
	...
	if (isQuoted)
	{
		...
		ELEKTRA_ADD_WARNINGF (ELEKTRA_WARNING_PARSING, parentKey,
				      "Unexpected end of line(%lu). unbalanced number of double-quotes in (%s)", lineNr, line);
	}
	else if (isCol)
	{
		if (line[len - 2] == '\n' || line[len - 2] == '\r')
	...
	{
		ELEKTRA_ADD_WARNINGF (ELEKTRA_WARNING_PARSING, parentKey, "Quoted field in line(%lu) has an unescaped double-quote: (%s)",
				      lineNr, line);
	}
		    ...
		    const char ** colNames)
{
	...
	if (fixColumnCount)
	{
		...
		{
			ELEKTRA_SET_ERRORF (ELEKTRA_ERROR_PARSING, parentKey, "illegal number of columns (%lu - %lu) in Header line: %s",
		...
		if (colCounter != columns)
		{
			if (fixColumnCount)
			{
				ELEKTRA_SET_ERRORF (ELEKTRA_ERROR_PARSING, parentKey,
						    "illegal number of columns (%lu - %lu) in line %lu: %s", colCounter, columns,
			...
			ELEKTRA_ADD_WARNINGF (ELEKTRA_WARNING_PARSING, parentKey, "illegal number of columns (%lu - %lu)  in line %lu: %s",
...
static int csvWrite (KeySet * returned, Key * parentKey, KeySet * exportKS, Key * colAsParent, char delim, short useHeader)
{
	...
	while ((cur = ksNext (returned)) != NULL)
	{
		...
		if (colCounter != columns)
		{
			ELEKTRA_SET_ERRORF (ELEKTRA_ERROR_PARSING, parentKey, "illegal number of columns (%lu - %lu) in line %lu",
					    colCounter, columns, lineCounter);
			fclose (fp);
			return -1;
		}
		++lineCounter;
