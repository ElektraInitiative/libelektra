...
static int elektraUnlockMutex (Key * parentKey ELEKTRA_UNUSED)
{
	...
	if (ret != 0)
	{
		ELEKTRA_ADD_WARNINGF (ELEKTRA_WARNING_RESOURCE, parentKey, "mutex unlock failed with message: %s", strerror (errno));
		return -1;
	}
	return 0;
#else
...
static void elektraCloseFile (int fd, Key * parentKey)
{
	if (close (fd) == -1)
	{
		ELEKTRA_ADD_WARNINGF (ELEKTRA_WARNING_RESOURCE, parentKey, "close file failed with message: %s", strerror (errno));
	}
}
	...
	case KEY_NS_SPEC:
		...
		if (needsMapping (testKey, errorKey))
		{
									    ...
									    errorKey)) == NULL)
			{
				resolverClose (p);
				keyDel (testKey);
				ELEKTRA_SET_ERROR (ELEKTRA_ERROR_RESOURCE, errorKey, "Could not resolve spec key");
				return -1;
			}
			else
			{
	...
	case KEY_NS_DIR:
		...
		if (needsMapping (testKey, errorKey))
		{
									    ...
									    errorKey)) == NULL)
			{
				resolverClose (p);
				keyDel (testKey);
				ELEKTRA_SET_ERROR (ELEKTRA_ERROR_RESOURCE, errorKey, "Could not resolve dir key");
				return -1;
			}
			else
			{
	...
	case KEY_NS_USER:
		...
		if (needsMapping (testKey, errorKey))
		{
									    ...
									    errorKey)) == NULL)
			{
				resolverClose (p);
				keyDel (testKey);
				ELEKTRA_SET_ERRORF (ELEKTRA_ERROR_RESOURCE, errorKey, "Could not resolve user key with conf %s",
	...
	case KEY_NS_SYSTEM:
		...
		if (needsMapping (testKey, errorKey))
		{
									    ...
									    ELEKTRA_RESOLVER_TEMPFILE_SAMEDIR, errorKey)) == NULL)
			{
				resolverClose (p);
				keyDel (testKey);
				ELEKTRA_SET_ERRORF (ELEKTRA_ERROR_RESOURCE, errorKey, "Could not resolve system key with conf %s",
...
int ELEKTRA_PLUGIN_FUNCTION (open) (Plugin * handle, Key * errorKey)
{
	...
	if (!path)
	{
		ELEKTRA_SET_ERROR (ELEKTRA_ERROR_RESOURCE, errorKey, "Could not find file configuration");
		return -1;
	}
...
error:
{
	...
	ELEKTRA_SET_ERROR (ELEKTRA_ERROR_RESOURCE, parentKey, errorText);
...
static int elektraCheckConflict (resolverHandle * pk, Key * parentKey)
{
	...
	if (fstat (pk->fd, &buf) == -1)
	{
		...
		strcat (errorText, "\" ");
		elektraAddIdentity (errorText);
		ELEKTRA_ADD_WARNING (ELEKTRA_WARNING_RESOURCE, parentKey, errorText);
		elektraFree (errorText);
...
static void elektraUpdateFileTime (resolverHandle * pk, int fd, Key * parentKey)
{
	...
	{
		ELEKTRA_ADD_WARNINGF (ELEKTRA_WARNING_LOGICAL, parentKey, "Could not update time stamp of \"%s\", because %s",
...
static int elektraSetCommit (resolverHandle * pk, Key * parentKey)
{
	...
	{
		ELEKTRA_SET_ERRORF (ELEKTRA_ERROR_RESOURCE, parentKey, "Could not rename file: %s", strerror (errno));
		ret = -1;
	}
	...
	{
		ELEKTRA_ADD_WARNINGF (ELEKTRA_WARNING_INSTALLATION, parentKey, "Could not sync directory \"%s\", because %s", pk->dirname,
				      strerror (errno));
	}
	closedir (dirp);
...
int ELEKTRA_PLUGIN_FUNCTION (set) (Plugin * handle, KeySet * ks, Key * parentKey)
{
	...
	if (pk->fd == -1)
	{
		...
		if (ksGetSize (ks) == 0)
		{
			...
			{
				ELEKTRA_SET_ERRORF (ELEKTRA_ERROR_RESOURCE, parentKey, "Could not remove file: %s", strerror (errno));
				ret = -1;
			}
...
static void elektraUnlinkFile (char * filename, Key * parentKey)
{
	...
	{
		ELEKTRA_ADD_WARNINGF (ELEKTRA_WARNING_RESOURCE, parentKey, "could not unlink the file \"%s\" because of \"%s\"", filename,
				      strerror (errno));
		errno = errnoSave;
	}
}
