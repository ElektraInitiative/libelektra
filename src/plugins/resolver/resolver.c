...
static int mapFilesForNamespaces (resolverHandles * p, Key * errorKey)
{
    ...
    switch (KEY_NS_SPEC)
    {
        case KEY_NS_SPEC:
            ...
            {
                if ((resolved = ELEKTRA_PLUGIN_FUNCTION (resolver, filename) (KEY_NS_SPEC, (p->spec).path, ELEKTRA_RESOLVER_TEMPFILE_SAMEDIR, errorKey)) == NULL)
                {
                    resolverClose (p);
                    keyDel (testKey);
        ...
        case KEY_NS_DIR:
            ...
            if (needsMapping (testKey, errorKey))
            {
                if ((resolved = ELEKTRA_PLUGIN_FUNCTION (resolver, filename) (KEY_NS_DIR, (p->dir).path, ELEKTRA_RESOLVER_TEMPFILE_SAMEDIR,  errorKey)) == NULL)
                {
                    resolverClose (p);
                    keyDel (testKey);
        ...
        case KEY_NS_USER:
            ...
            if (needsMapping (testKey, errorKey))
            {
                if ((resolved = ELEKTRA_PLUGIN_FUNCTION (resolver, filename) (KEY_NS_USER, (p->user).path, ELEKTRA_RESOLVER_TEMPFILE_SAMEDIR, errorKey)) == NULL)
                {
                    resolverClose (p);
                    keyDel (testKey);
        ...
        case KEY_NS_SYSTEM:
            ...
            if (needsMapping (testKey, errorKey))
            {
                if((resolved = ELEKTRA_PLUGIN_FUNCTION (resolver, filename) (KEY_NS_SYSTEM, (p->system).path, ELEKTRA_RESOLVER_TEMPFILE_SAMEDIR,  errorKey)) == NULL)
                {
                    resolverClose (p);
                    keyDel (testKey);
