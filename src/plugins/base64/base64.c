...
static int unescape (Key * key, Key * parent)
{
	...
	if (!unescaped)
	{
		ELEKTRA_SET_ERROR (ELEKTRA_ERROR_RESOURCE, parent, "Memory allocation failed");
		return -1;
	}
...
static int decode (Key * key, Key * parent, bool metaMode)
{
	...
	{
		// Decoding error
		ELEKTRA_ADD_WARNINGF (ELEKTRA_WARNING_PARSING, parent, "Not Base64 encoded: %s", strVal);
	}
	else if (result == -2)
	{
		// Memory error
