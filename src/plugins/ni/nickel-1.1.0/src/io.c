...
static int PutString(FILE * restrict f, const char * restrict str, int str_len,
                     int is_key, int is_section);

//Puts a single UTF-8 character into the file.
static int PutUtf8Char(FILE * restrict f,
 ...
 */
Ni_PRIVATE int GetNextIdentifier(file_buf * restrict fb,
                                 char * restrict idfr_out,
                                 int * restrict len_out,
                                 int * restrict level_out)
{
   ...
   while(state != ST_DONE) //until we decide to stop
   {
      ...
      case ST_IN_U:
         if (c == T_EOL)      { state = ST_DONE;           //if eof or eol, strip trailing space, we done
                               ...
                               { chkgr(c);            }   //if not escaping, check whether it was graphical
                               put(c);                  } //and regardless, put something in the output
         break;
 ...
 * to f if it fails.
 */
Ni_PRIVATE int PutSection(FILE * restrict f,
                          const char * restrict name, int name_len, int level)
{
   int i;
   int success = 0;
 ...
 * it fails.
 */
Ni_PRIVATE int PutEntry(FILE * restrict f,
                        const char * restrict key, int key_len,
                        const char * restrict value, int value_len, int level)
{
   int i;
   int success = 0;
