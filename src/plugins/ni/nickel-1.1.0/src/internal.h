...
#define FILE_BUF_INIT {NULL, Ds_VECTOR_INIT, 0} //initializer
...
int GetNextIdentifier(file_buf * restrict fb, char * restrict idfr_out,
                      int * restrict len_out, int * restrict level_out);
int GetValue(file_buf * restrict fb, Ds_str * restrict value_out);
int PutSection(FILE * restrict f, const char * restrict name, int name_len,
               int level);
int PutEntry(FILE * restrict f, const char * restrict key, int key_len,
