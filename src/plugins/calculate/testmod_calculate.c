...
static void test_set_else_val()
{
	Key *parentKey = keyNew("user/tests/calculate", KEY_VALUE, "", KEY_END);
	KeySet *ks = ksNew(5,
	keyNew("user/tests/calculate/sum", KEY_VALUE, "0", KEY_META, "check/calc", ":= + bla/val1 + bla/val2 bla/val3", KEY_END),
	keyNew("user/tests/calculate/bla/val1", KEY_VALUE, "100", KEY_END),
	keyNew("user/tests/calculate/bla/val2", KEY_VALUE, "50", KEY_END),
	keyNew("user/tests/calculate/bla/val3", KEY_VALUE, "3", KEY_END),
		KS_END);
	...
	succeed_if(plugin->kdbSet(plugin, ks, parentKey) == (1), "error");
	Key *key;
	key = ksLookupByName(ks, "user/tests/calculate/sum", 0);
	succeed_if(atoi(keyString(key)) == 153, "set value failed");
	ksDel(ks);
	keyDel(parentKey);
	PLUGIN_CLOSE();
