...
int elektraZeroMqSendSet (Plugin * handle, KeySet * returned ELEKTRA_UNUSED, Key * parentKey ELEKTRA_UNUSED)
{
	...
	case -1:
		...
		ELEKTRA_ADD_WARNING (ELEKTRA_WARNING_TIMEOUT, parentKey, "could not connect to hub. Please start hub using `kdb run-hub-zeromq`.");
		break;
	case -2:
		// subscription timeout - no applications are listening for notifications, can be ignored
		break;
	default:
		ELEKTRA_ADD_WARNING (ELEKTRA_WARNING_INSTALLATION, parentKey, "could not send notifications");
		break;
	}
