...
int elektraZeroMqSendSet (Plugin * handle, KeySet * returned ELEKTRA_UNUSED, Key * parentKey ELEKTRA_UNUSED)
{
	...
	case -1:
		...
		ELEKTRA_ADD_WARNING (TIMEOUT_CODE, parentKey, "could not connect to hub. Please start hub using `kdb run-hub-zeromq`.");
		break;
	case -2:
		// subscription timeout - no applications are listening for notifications, can be ignored
		break;
	default:
		ELEKTRA_ADD_WARNING (INSTALLATION_CODE, parentKey, "could not send notifications");
		break;
	}
