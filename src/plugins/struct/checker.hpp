...
public:
	...
	void check (kdb::KeySet & ks) override
	{
		...
		if (!confRoot)
			throw "StructChecker: No confRoot found";

		kdb::Key cur;
		kdb::Key root = ks.next ();
		if (!root)
			throw "StructChecker: No root key found";
		...
		while ((cur = ks.next ()))
		{
			...
			if (!searchKey)
				throw "StructChecker: More keys found than structure should have";
			if (!cur.isDirectBelow (root))
				throw "StructChecker: key is not direct below";
		...
		if (config.next ())
			throw "StructChecker: There should be more elements in the structure";
	}
};
...
public:
	...
	void check (kdb::KeySet & ks) override
	{
		...
		if (!root)
			throw "ListChecker: no root key found";

		while ((k = ks2.next ()))
		{
			if (!root.isDirectBelow (k))
				throw "ListChecker: key is not direct below";
