...
int elektraPortInfo(Key * toCheck, Key * parentKey) {
	...
	if (*endptr == '\0') {
		...
		if (service == NULL) {
			ELEKTRA_SET_ERRORF(206, parentKey, "Could not find service with name %s on key %s",
	...
	if (server == NULL) {
		ELEKTRA_SET_ERRORF(208, parentKey, "Could not connect to %s: No such host",
						   hostname);
		return -1;
	}
	...
	if (connect(sockfd,(struct sockaddr *) &serv_addr,sizeof(serv_addr)) == 0) {
		close(sockfd);
		ELEKTRA_SET_ERRORF(207, parentKey, "Port %s is already in use which was specified on key %s",
						   keyString(toCheck), keyName(toCheck));
		return -1;
	}
	close(sockfd);
