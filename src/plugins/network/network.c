...
int elektraPortInfo (Key * toCheck, Key * parentKey)
{
	...
	if ( *endptr == '\0' )
	{
		...
		{
			ELEKTRA_SET_ERRORF(201, parentKey, "Port %ld on key %s was not within 0 - 65535", portNumber, keyName (toCheck));
			return -1;
		}
	...
	else
	{
		...
		if ( service == NULL)
		{
			ELEKTRA_SET_ERRORF(202, parentKey, "Could not find service with name %s on key %s", keyString (toCheck),
					   keyName (toCheck));
			return -1;
		}
	...
	if ( server == NULL)
	{
		ELEKTRA_SET_ERRORF (204, parentKey, "Could not connect to %s: No such host", hostname);
		return -1;
	}
	...
	{
		close (sockfd);
		ELEKTRA_SET_ERRORF (203, parentKey, "Port %s is already in use which was specified on key %s", keyString (toCheck),
				    keyName (toCheck));
		return -1;
	}
	close (sockfd);
...
int elektraNetworkGet (Plugin * handle ELEKTRA_UNUSED, KeySet * returned, Key * parentKey ELEKTRA_UNUSED)
{
	...
	ksAppend (returned,
		  n = ksNew (30,
			     keyNew ("system/elektra/modules/network", KEY_VALUE, "network plugin waits for your orders", KEY_END),
