...
- [ ] First line explains briefly what the function does - [ ] append to 'to the given `cursor`'
...
- [ ] `@since` - [ ] add
- [ ] `@ingroup` - [ ] add
- [ ] `@see` - [ ] add `ksCurrent()` to `ksNext()` - [ ] move `ksGetCursor` to its own line
...
- [ ] All possible error states are covered by tests - [ ] `ks == NULL` - [ ] `cursor == -1`
