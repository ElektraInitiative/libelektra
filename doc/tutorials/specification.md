...
kdb file user:/sw/org/app/#0/current
...
```sh
kdb meta-set spec:/sw/org/app/\#0/current/server/port type unsigned_short
```
...
```sh
kdb meta-set spec:/sw/org/app/\#0/current infos/plugins "ni type network"
```
...
```sh
kdb set user:/sw/org/app/\#0/current/server/port 65536

#  Sorry, 1 warning was issued ;(
#  1: Module network issued the warning C03200:
# 	Validation Semantic: Port 65536 on key user:/sw/org/app/#0/current/server/port was not within 0 - 65535
# Sorry, module type issued the error C03200:
# Validation Semantic: The type 'unsigned_short' failed to match for 'user:/sw/org/app/#0/current/server/port' with string '65536'
```
...
kdb get /sw/org/app/\#0/current/backup/date
#> 2021-11-01
```
...
```sh
kdb set user:/sw/org/app/\#0/current/backup/date "2022-04-03"
#> Create a new key user:/sw/org/app/#0/current/backup/date with string "2022-04-03"
...
```sh
sudo kdb mount /etc/spec.ni spec:/sw/org/app/\#0/current ni
sudo cp ./spec.ni $(kdb file spec:/sw/org/app/\#0/current)
```
...
```sh
sudo rm -v `kdb file spec:/sw/org/app/#0/current`
rm -v `kdb file user:/sw/org/app/#0/current`
sudo rm -v /usr/share/elektra/specification/spec.ni
sudo rm -v /etc/spec.ni
rm -v ./spec.ni

sudo kdb umount /sw/org/app/#0/current
sudo kdb umount spec:/sw/org/app/#0/current
```
