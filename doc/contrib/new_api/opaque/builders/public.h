...
#define ELEKTRA_KEYNAME(ns_, name_) elektraNameNew ((ns_), (name_), sizeof (name_))

#define ELEKTRA_METANAME(name) ELEKTRA_KEYNAME (ELEKTRA_NS_META, (name))
#define ELEKTRA_KEYVALUE_STRING(s) (&(ElektraValue){ .value = (s), .size = strlen ((s)) })
#define ELEKTRA_KEYVALUE_PTR(v) (&(ElektraValue){ .value = &(v), .size = sizeof ((v)) })
...
ElektraEntry * elektraEntryBuild (const ElektraName * name, const ElektraValue * value, ElektraSet * meta)
{
	...
	elektraSetValue (key, value->value, value->size);
