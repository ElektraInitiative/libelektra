...
// elektraSetInsertAndRelease() creates new reference and then destroys existing reference
...
void fooConsume (ElektraEntry * e1, ElektraEntry * e2)
{
	...
	// we don't need e2
	// -> insert with release
	elektraSetInsertAndRelease (set, e2);
...
int main (void)
{
	...
	ElektraEntry * e1 = elektraEntryNew (&(ElektraName){ .ns = ELEKTRA_NS_SYSTEM, .name = "foo\0bar\0baz", .size = 14 });
	// REFS: e1.refs = 1
	ElektraEntry * e2 = elektraEntryNew (&(ElektraName){ .ns = ELEKTRA_NS_SYSTEM, .name = "foo\0bar\0baz", .size = 14 });
	...
	ElektraEntry * e3 = elektraEntryNew (&(ElektraName){ .ns = ELEKTRA_NS_SYSTEM, .name = "foo\0bar\0baz", .size = 14 });
	// REFS: e3.refs = 1
	ElektraEntry * e4 = elektraEntryNew (&(ElektraName){ .ns = ELEKTRA_NS_SYSTEM, .name = "foo\0bar\0baz", .size = 14 });
	...
	ElektraEntry * e5 = elektraEntryNew (&(ElektraName){ .ns = ELEKTRA_NS_SYSTEM, .name = "foo\0bar\0baz", .size = 14 });
	// REFS: e5.refs = 1
	ElektraEntry * e6 = elektraEntryNew (&(ElektraName){ .ns = ELEKTRA_NS_SYSTEM, .name = "foo\0bar\0baz", .size = 14 });
	...
	ElektraEntry * e7 = elektraEntryNew (&(ElektraName){ .ns = ELEKTRA_NS_SYSTEM, .name = "foo\0bar\0baz", .size = 14 });
	// REFS: e7.refs = 1
	ElektraEntry * e8 = elektraEntryNew (&(ElektraName){ .ns = ELEKTRA_NS_SYSTEM, .name = "foo\0bar\0baz", .size = 14 });
