...
int main (void)
{
	...
	ElektraEntry * k1 = elektraEntryNew (ELEKTRA_NS_SYSTEM, (ElektraName){ .name = "foo\0bar\0baz", .size = 14 });

	// get name from key
	__attribute__ ((unused)) ElektraName name1 = elektraEntryGetName (k1);

	// create local name
	ElektraName name2 = { .name = "foo", .size = 4 };

	// Create key using existing name
	ElektraEntry * k2 = elektraEntryNew (ELEKTRA_NS_USER, name2);
...
#pragma GCC diagnostic pop
	...
	__attribute__ ((unused)) ElektraEntry * l1 = elektraSetGet (ks1, elektraSetLookupEntry (ks1, k1));

	// ... with local name ...
	// Note: Using ElektraEntryGetName (k2) would also be unsafe, because we've given up that reference
	ElektraEntry * l2 = (ElektraEntry *) elektraEntryIncRefCount (elektraSetGet (ks1, elektraSetLookup (ks1, ELEKTRA_NS_USER, name2)));
	// ... or inline name
	ElektraEntry * l3 = elektraSetGet (ks1, elektraSetLookup (ks1, ELEKTRA_NS_CASCADING, (ElektraName){ .name = "foo", .size = 4 }));
