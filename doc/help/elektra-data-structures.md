...
The allocation doubles the space if the `Vstack` is full and reduces the space by the half
if the memory usage gets under a quarter.
...
The `Vheap` is a virtual heap implementation with dynamical memory allocation.
