...
- The [clang-format wrapper script](../../scripts/dev/reformat-source) now also checks the supported maximum version of Clang-Format. _(René Schwaiger)_
- The script [`reformat-shfmt`](https://master.libelektra.org/scripts/dev/reformat-shfmt) now also reformats shell support files (`*.in`) in the [`scripts`](https://master.libelektra.org/scripts) folder. _(René Schwaiger)_
