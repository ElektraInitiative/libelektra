...
- to prepare Elektra for [1.0.0](https://github.com/ElektraInitiative/libelektra/milestone/12),
...
100% free software (BSD licensed). We do not plan
to make any part proprietary. We only introduce a
way for paid customers to get desired features or
help more quickly.
...
as its storage backend and lazily stores keysets from previous ´kdbGet()´
calls. We added initial support for the default resolver and multifile resolver.

With large or many configuration files, the cache brings amazing performance improvements:
Let us say, you have 649 INI configuration files mounted with the multiresolver, completely
...
With the cache, the whole operation now takes less than 0.5 seconds.
...
[gopts](https://www.libelektra.org/plugins/gopts) is a new plugin that integrate support for command-line options to applications:
...
- `kdbEnsure` right now ignores the `infos/recommends` and `infos/needs` metadata of plugins, so you have to explicitly take care of
  dependencies.
...
We were able to reduce the former 214 to 9 error codes.
