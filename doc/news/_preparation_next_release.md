...
- Removed obsolete `ksLookupByString` and `ksLookupByBinary`, as well as deprecated `KDB_O_*` options. _(Philipp Gackstatter)_
...
- Rewrote the `KDBError` to follow the specification fully and in particular allow catching out of memory errors by catching resource errors.
- Added a `keyset!` macro to easily create a keyset with many keys in a single invocation.
