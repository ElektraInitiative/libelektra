...
but LCDproc itself is now well-maintained by Harald Geyer on
...
1. We (Elektra Initiative) remove as much code as possible from LCDproc's
   code base.
...
- [Profile support](https://www.libelektra.org/plugins/profile): Having multiple
  complete configuration settings you can easily choose from.
...
- Elektra's website can be used to share LCDproc's configuration files,
  ...
  to mount files from the website.
...
- Kurt: Order Preserving Minimal Hash Map
