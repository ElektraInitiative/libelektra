...
5. Allow arbitrary sequences and let each plugin deal with it on a case-by-case basis.
   ...
   However, this could be paired with the [COW semantics](../1_in_discussion/internal_cache.md) so the memory toll would not be that big of a deal.
...
6. Don't restrict sequences further and provide a common framework to handle change tracking correctly.
   ...
   This approach can also be paired with [COW semantics](../1_in_discussion/internal_cache.md), so that memory toll will be kept low.
...
- [Internal KeySet Cache](../1_in_discussion/internal_cache.md)
- []()
